...
| + Plan 09 (per-item markers + DOM-reusing first mutation) | dev.semantic-ui.com | 83.3, 83.8, 95.6 | **~84** | 907188416 | Hydrate cost unchanged (O(1) per item preserved). First-mutation filter 1000→114 cards ~70 ms initially (silent-fallback bug). Fixed in follow-up commit: key stringification — adoption now verified via Node-identity test (`preNode === postNode` for matching items), first-mutation 1000→114 cards drops to **~23 ms**. |
