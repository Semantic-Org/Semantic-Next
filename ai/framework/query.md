...
> **Related:** [Mental Model](/ai/framework/mental-model.md) • [Component Guide](/ai/framework/creating-components.md) • [Quick Reference](/ai/framework/quick-reference.md)  
...
- `clippingParent(options)` - Get element that clips visual bounds (`{ pierceShadow }`)
- `containingParent()` - Get simple containing parent (offsetParent)
- `positioningParent(options)` - Get accurate positioning context parent (`{ calculate, pierceShadow }`)
- `scrollParent(options)` - Get nearest scrollable container (`{ all, pierceShadow }`)
