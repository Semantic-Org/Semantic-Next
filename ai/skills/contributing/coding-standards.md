...
- **Sketch the tree before the first file.** A package's layout is particular to the package, so it is written down as a tree like the one above, in the plan or the charge, before code exists, and judged by one test: each folder's listing reads as a list of the things its name says it holds. A layout that is discovered on review costs a round of renames every time.
- **A folder named for a kind of thing is homogeneous.** `providers/`, `types/`, `subtemplates/`, `engines/`: one entry per thing, every entry the same kind, and nothing else beside them. A helper the entries share goes in `helpers/`, never in a `shapes.js` sitting among them; an entry's own sub-parts nest inside its folder, never as a sibling folder of a different kind. The failures this rule names were real: providers left in a mixed folder, a types folder holding types and their parts, and a bag of exported functions filed among the subtemplates it served.
- **`helpers/` is bags of functions, nothing else.** Free functions grouped one concern per file. A class, a registry or a catalog entry is never a helper, and a helper never carries state.
- **The top of `src/` holds the important files only.** The barrel, the nouns, and the entries a reader comes to the package for. What serves them, the error door, the logger, the copy table, an address normalizer, lives under `helpers/`, so `ls src/` reads as the package's surface and nothing else.
