---
title: AI Writing Tropes Reference
description: Reference catalog of 30+ named AI writing anti-patterns organized by category — word choice, sentence structure, paragraph structure, tone, formatting, and composition. Load when editing documentation prose to identify specific patterns.
keywords: [AI tropes, anti-patterns, writing quality, slop, word choice, sentence structure, tone, formatting, composition]
audience: docs
skill: docs-ai-tropes
type: skill
---
...
> **Skill:** `docs-ai-tropes`
> **Purpose:** Reference catalog of named AI writing anti-patterns — load when editing prose to identify specific patterns

**Companion to:** `docs-writing` (load that first for the editing framework and quality baseline)

Any pattern used once might be fine. The problem is when multiple tropes cluster or a single trope repeats. Write like a human: varied, imperfect, specific.
...
The "It's not X -- it's Y" pattern, often with an em dash. The single most commonly identified AI writing tell. AI uses this to create false profundity by framing everything as a surprising reframe. One in a piece can be effective; ten in a blog post is an insult to the reader.
...
Tacking a present participle ("-ing") phrase onto the end of a sentence to inject shallow analysis. The model attaches significance or legacy to mundane facts using phrases like "highlighting its importance", "reflecting broader trends", or "contributing to the development of...".
...
After making a claim, AI illustrates it with a stream of verbless gerund fragments — standalone sentences with no grammatical subject. The first sentence already said everything. The fragments add nothing except word count and that familiar AI cadence.
...
Excessive use of very short sentences or sentence fragments as standalone paragraphs for manufactured emphasis. RLHF training has pushed models toward one thought per sentence, no mental state-keeping required. No real person writes first drafts this way.
...
The patronizing analogy. AI constantly reaches for "Think of it as..." or "It's like a..." to simplify concepts. Often produces analogies that are less clear than the original concept.
...
The classic AI invitation to futurism. Begins with "Imagine" followed by a list of wonderful things that will happen if the reader agrees with the premise.
...
Simulated self-awareness or honesty that reads as performative. The model pretends to break the fourth wall or admit a bias. Real vulnerability is specific and uncomfortable; AI vulnerability is polished and risk-free.
...
Attributing claims to unnamed authorities instead of being specific. AI loves to invoke "experts", "observers", "industry reports" without naming anyone. It also inflates the quantity of sources.
...
AI clusters invented compound labels that sound analytical without being grounded. It appends abstract problem-nouns (paradox, trap, creep, divide, vacuum, inversion) to domain words and uses them as if they're established terms. Multiple such labels in the same piece is a strong signal of AI slop.
...
Every bullet point or list item starts with a bolded phrase or sentence. Almost nobody formats lists this way when writing by hand. Telltale sign of AI-generated documentation, blog posts, and README files.
...
Repeating entire sections or paragraphs verbatim within the same piece. This happens when the model loses track of what it has already written, especially in longer pieces.
...
- "Despite their promising applications, pyroelectric materials face several challenges that must be addressed."
...
|-------|---------|-------------|
| **Writing Docs** | `use_skill: docs-writing` | Quality baseline, editing strategy, and voice standards |
| **Authoring Standards** | `use_skill: docs-authoring-standards` | Page structure — headings, frontmatter, code blocks |
