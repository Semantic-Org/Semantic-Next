...
- **Spec field name.** `observeOptions: false` is the proposed flag. Alternatives considered: `staticOptions: true` (positive framing), `observe: false` (shorter). Lean: keep `observeOptions: false` — reads naturally next to the existing options array, opts out of the default behavior rather than introducing a new positive flag.
- **Icon alias post-trim impact.** Current count is 4,920 attributes pre-alias-trim. The icon-alias-audit (ROADMAP `P1`) will reduce this to ~400-600 names + ~2x compound generation = ~1,200. The bottleneck shrinks but doesn't disappear. The opt-out is still worth it post-audit because 1,200 observed attrs per icon × 200 icons on a docs page is still meaningful work.
- **Multi-value option attributes.** Icon's `<ui-icon home>` is single-valued (one icon name). If a future spec uses `observeOptions: false` for a multi-valued attribute (e.g. multiple variants on one element), the `connectedCallback` resolution loop would need to track all matched attrs rather than `break`-ing. Out of scope for icon, worth flagging in the spec doc when this lands.

## Dependencies

- None blocking. Builds on existing spec system. Lands cleanly before or after [Icon Alias Audit](icon-alias-audit.md) — they compound.

## Status

`scoped` — concrete file list, migration is non-breaking, design decisions locked. Ready to execute.
