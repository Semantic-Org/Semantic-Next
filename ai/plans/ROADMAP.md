...
- [Release 0.18.0](active/release-0-18-0.md) — [PR #122](https://github.com/Semantic-Org/Semantic-Next/pull/122) `docs/shippable` (menu trimming + audit pass pending). Ships the next tagged release; last was 0.17.0 in November. Reactivity Functional Surface (2a) has landed; remaining gate is the docs + types pass.
---
...
- [Data Sync Layer](icebox/sync.md) — realtime data layer (collections, publications, drafts, cursor-core protocol). Design corpus at `ai/research/sync/`, gate-vetted. Post-1.0 by design — simmering until the component catalog ships. Schema language extracts early via Phase 2g.
- [Data Sync Dogfooding](icebox/sync-dogfooding.md) — preview/staging/prod hosting for the sync backend via a `sync` Deploy Bot target (Fly + Neon branch, teardown). Economics in `ai/research/sync/infra/hosting.md`. Gated on the sync server packaging and the Deploy Bot landing on `main`.
