...
- [Template Match Blocks](active/template-match-blocks.md) — [PR #261](https://github.com/Semantic-Org/Semantic-Next/pull/261) `{#match}`/`{is}`/`{else}` value-based branching across compiler, native, lit, and SSR.
---
...
| 2e | [Template Match Blocks](active/template-match-blocks.md) | 8-16h (1-2d) | pair | scoped | `{#match}`/`{is}`/`{else}` — value-based branching. Replaces verbose `{#if is x 'a'}...{else if is x 'b'}` chains. |
