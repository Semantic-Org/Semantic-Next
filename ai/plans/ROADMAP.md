...
| 2a.4 | [Reactivity Surface Rename](reactivity-surface-rename.md) | 20-30h (3-5d) | pair | scoped | Lowercase free-function surface (`signal`, `reaction`, `computed`, `afterFlush`, `flushed`, `batch`, `flush`, `nonreactive`, `guard`) replacing class statics. One canonical form per operation, no aliases. Multi-PR sweep across `packages/`, `src/`, `docs/`, `docs/src/examples`. Blocked on `2a` (PR #150) and `2a.3`. **Final reactivity-side work before 1.0.** |
