...
- **Tier 1 / Tier 2 boundary.** When does a subtemplate need `defaultSettings`? The 75/25 split is a guess; in practice it depends on whether `createComponent` reads parent-derived data reactively. Naive subtemplates that only consume data via templates don't need it. A clear authoring guideline matters more than a strict rule.
- **Merged proxy fallback cost.** Every `settings.X` lookup walks own settings first, then falls through to parent web component settings. For deeply-passed data (`settings.foo.bar.baz`) this is one proxy hit + plain property access. Worth verifying performance under common access patterns before committing — likely fine, but cheap to measure.
- **Interaction with `safety: 'none'` (Signal Performance).** Shadow signals in subtemplate settings are framework-internal — they should use `safety: 'none'` (matching the pattern in `fine-grained-reactivity.md` and `createSettingsProxy`) once that preset lands. Without it, every `peek()` on the shadow signal pays the deep-clone cost.
- **Step 6 verification.** The todo-item example referenced (`docs/src/examples/framework/todo-list/todo-item.js`) needs verification that the file still exists in that form before this plan executes.
