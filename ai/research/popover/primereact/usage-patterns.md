...
- Comprehensive accessibility section with ARIA roles and keyboard navigation
...
| Escape key to close | ✅ | `closeOnEscape` prop enables ESC key dismissal. Documented behavior |
...
| Stateful trigger | ⚠️ | No built-in toggle state visualization on trigger. Component adds `aria-expanded` attribute to trigger |
