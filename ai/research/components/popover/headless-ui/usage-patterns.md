...
Excellent - Comprehensive documentation with practical code examples, clear API reference, and detailed explanations of positioning, transitions, and accessibility features. Strong focus on real-world usage patterns and integration with Tailwind CSS. Well-structured progressive examples showing basic to advanced patterns.
...
| Modal mode | ✅ | Native | `modal` prop enables scroll locking |
...
<PopoverPanel
  anchor="bottom"
  modal  // Enables scroll locking
  className="bg-white shadow-lg rounded-lg p-4"
>
  {/* Enhanced accessibility with scroll locking */}
</PopoverPanel>
```
...
```jsx
<Popover>
  <PopoverButton>Open Form</PopoverButton>
  <PopoverPanel anchor="bottom">
    {({ close }) => (
      <form onSubmit={async (e) => {
        e.preventDefault()
        await submitForm() // Async operation
...
| `modal` | Boolean | `false` | Enables scroll locking |
...
```typescript
{
  open: boolean  // Whether backdrop is showing
}
```
...
- `data-closed` - Applied when popover is closed (implicit, absence of data-open)
...
### useClose()
Returns a `close` function that closes the nearest ancestor PopoverPanel.
...
**Parameters:**
- `focusRef?` (optional): Element reference to focus after closing

**Usage Notes:**
- Must be used within a Popover component tree
- Useful for deeply nested components that need close functionality
- Provides same functionality as render prop `close()` function
...
### 11. Modal Mode
The `modal` prop enables scroll locking:
- Prevents body scroll when panel is open
- Useful for full-screen or prominent popovers
...
- Progressive examples showing basic to advanced patterns
...
**vs. Menu Components:**
- Persistent on interaction (menus close on selection)
- More flexible content structure
- Different keyboard patterns
...
<Popover>
  <PopoverButton className="my-button my-button--primary">
    Toggle
  </PopoverButton>
  <PopoverPanel className="my-panel my-panel--floating">
    {/* content */}
  </PopoverPanel>
</Popover>
```
  ...
  <PopoverPanel>
    {({ close }) => (
      <form onSubmit={async (e) => {
        e.preventDefault()
        await saveData()
        close() // Close after successful operation
      }}>
        {/* form fields */}
      </form>
    )}
...
- `unmount={false}` preserves state but keeps elements in DOM
