...
- **Named mutators** — the graduation for gated/validated/multi-collection writes. todomvc needs none; see landings-table.
...
- `todos.js` — schema + a publication + two actions, ~50 lines. No CRUD mutators: the direct `db.todos` writes and the universal `Todos.update`/`insert`/`remove` are the write surface. (The multi-collection / audit-log mutator demonstration lives in landings-table, the graduation steelman.)
