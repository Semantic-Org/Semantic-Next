...
- Genuinely small value-lists (permit numbers, gear tags) keep whole-array semantics. The schema declares which arrays are *keyed*, and the test is concurrency rather than size — any array two people or two stations can be inside at once gets keyed. The framework mints row ids for declared keyed arrays at insert, and id-addressing applies there. Un-keyed value-lists are common by count and rare by concurrency.
...
- **Implicit save is the law of the hot paths.** Tally and grading entry commits through mutators `oninput`, not on blur, debounced ~300ms trailing (~). No save buttons — a tallyman calling weights off a scale does not stop to save, and with a filing deadline running an unsaved buffer is a compliance risk rather than an inconvenience. The user's mental model is "recorded when entered" — every protocol and UX decision answers to this.
...
- **Human tally is slow, machines are not.** A tote crosses a dockside scale every 5-15 seconds, so a tally station commits at ~0.1-0.2/sec and ten stations together stay under 2/sec. The rate comes from the automated graders, which weigh and sort individual fish at a few per second and post continuously: eight lines at ~4/sec each put **~33 commits/sec on one trip document**, ≈ 5-10KB/s, ≈ 33 delta frames/sec/subscriber. Frame rate, not bandwidth, is the watch variable. Ten times the grading capacity, a large processor running every line against one landing, is the ~330 frames/sec/subscriber ceiling.
...
- Detail-page reaction pressure: the trip-level catch grid binds on the order of **1,800 leaves** (300 visible rows of the trip's 1,000-3,000 × ~6 fields), so under per-doc dep granularity 33 frames/sec × 1,800 bindings ≈ **60,000 reaction runs/sec/viewer** — two orders of magnitude past the form-page case. This is the figure that makes per-row and per-field deps mandatory rather than an optimization.
...
| Leaf paths per doc | ~10,000 for a mid-range trip, past 25,000 at the high-haul-count tail. Depth budget ~10, leaf mass expected around depths 4-7. Conflict map prunes to replay horizon | |
| Hauls per trip | 50-200 | |
| Catch rows per haul | 8-20 | |
| Catch rows per trip | 1,000-3,000 | |
