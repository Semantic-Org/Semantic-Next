...
- **vocabulary** named permissions that derive from each other through the schema compute graph. this
  ...
  scoped-enum permission (`['subscribed', 'group', 'all']`), never one column per level.
...
handshake rather than per write is what spares this design a principal lookup on every mutation.
...
behind different auth gates (`trips.summary` vs `trips.admin`), never a per-subscriber projection
...
#### where this is structurally better than gating a CRUD library from outside
...
- **native deep-path field conditions** `enforcePatch` already walks the full dotted touched-path set
  ...
  deep-path allow / deny-lists are native, where a permission layer bolted onto a collection API sees
  only top-level field names, so any deeper rule has to be hand-built. our enforce step resolves the descriptor at
...
- **one authoritative chokepoint** `runMutator` (the `enforce` step) gates every write path, so there
  is no second mechanism to keep in agreement, the split a bolted-on layer forces when declarative rules
  gate client writes and server writes go around them. one place authorizes a write.
...
one column per level (`region_ids`, `team_ids`, and so on): depth is data, so adding a layer touches
zero callsites. its shape:
...
- **resolution lives behind one primitive** record-scope composed with principal-scoped-grants resolves
  in one place, never inlined per-callsite, since level-resolution inlined at every callsite is what
  turns adding a hierarchy layer into a codebase-wide edit. that single-primitive, never-name-a-level
  invariant is the fixed contract here.
