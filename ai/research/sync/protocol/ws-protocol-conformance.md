...
A dashboard channel `landings.table?…` declares `freshness: '5s'`. The subscribed writer updates a landing's status.
...
```json
[ { "type": "delta", "channel": "landings.table?{\"page\":1}", "cursor": "3:119",
    "changes": [{ "collection": "landings", "id": "lnd-7", "op": "change",
                  "fields": { "status": "approved" } }] } ]
```
```json
{ "type": "result", "id": 41, "status": "ok",
  "positions": { "landings.table?{\"page\":1}": "3:119" } }
```
