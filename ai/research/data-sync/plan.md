...
First datapoint (sync-poc, 2026-06-12): kernel standalone 8.8KB gzip / 8.0KB brotli (6.7 / 6.1 with utils external) — pool, matcher, protocol client, outbox, rebase, and persistence in, though persistence is budgeted as an opt-in tier; absent from the measurement: txid grouping, parking, projections, coverage, multi-tab, the SSE fallback, schema. A quarter of the target with the protocol spine in.
