...
This refines three `plan.md` sections (Client Store, Write Path, Channels) — the specific amendments are listed at the end. The two steelmans are its worked examples: `todomvc/` (a pool collection — the `db.<name>` case) and `invoices-table/` (a search window + named mutators — the graduation case).
...
**Window channels surface a handle, not an array.** A searchIndex / recompute channel computes membership and meta server-side (order, `total`, `pages`) — the pool cannot reconstruct it. Its subscription surfaces the *handle*: iterable for `{#each}`, with `.total` / `.pages` / `.ready` as properties (`{invoices.total}`, `{not invoices.ready}`). invoices-table is this case; todomvc is the pool-collection case. So "subscriptions surface their data" holds for both — the data face is an array for a pool, a window handle for a recompute channel.
...
Invoices.action('approve',    { permission, schema, async run() { … } });
Invoices.publish('byId',      { … });
