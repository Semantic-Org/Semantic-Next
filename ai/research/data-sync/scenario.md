...
- **Expected impact:** detail channels (`records.byId`) receive one delta each — no storm. Overview/searchIndex channels hit log collapse and reset to snapshot, by design. The real cost is **fan-out, not matching** *(corrected by implementation measurement, 2026-06-19)*: selector-matching stays effectively flat — ~14% CPU at 6000 channels — so the binding cost is delta fan-out, mitigated dominantly by **channel scope** (a write reaches roughly one tenant's subscribers, not the whole workspace). The match index is still worth building for routing correctness at extreme channel counts, but it is not the decisive capacity lever this once anticipated; off-peak bounding still helps.
