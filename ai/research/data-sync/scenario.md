...
- The corpus follows a power law: dozens of collections with a low median field count, the large majority small, one dominant collection, one config-blob sub-monster, then a long tail. The "one monster plus small references" assumption is the real shape, not a simplification. The "one monster plus small references" assumption is the real shape, not a simplification.
...
| Leaf paths per doc | several thousand at actual fill (arrays multiply schema capacity). Depth to ~10, leaf mass at mid-depths. Conflict map prunes to replay horizon | |
| Projected wire size, full `records.byId` | ~150-200KB, mass concentrated in a few top-level subtrees — those keys carry the binding re-fire pressure | |
