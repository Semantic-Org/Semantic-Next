...
exists, and per-field permission files ran at production scale for years in the systems
this design draws on. what actually died in the Meteor era was ad-hoc callback validation
...
mutators: {
  ...
  // server file:  Invoices.mutator('settle', { mutate(doc) { env.KEY; ... } });
}
...
- **the graduated-trust posture**: denying raw CRUD wholesale while allowing named verbs
  whose effects cannot be forged. rung 2 exists precisely for this; the census question is
