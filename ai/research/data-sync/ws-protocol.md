...
Extends [`plan.md`](plan.md) (Protocol Sketch, Sync Loop, Channels). This is the wire contract a second implementer builds from with none of the design dialogue in context: every standing ruling absorbed into the text, evidence named per clause. The spec is the contract and conformance is the portability story — portable because conformance-tested, not because small (Decision 9). The conformance-case catalog lives in [`ws-protocol-conformance.md`](ws-protocol-conformance.md); clauses below cite case ids as *(conformance: `id`)*.

Everything here preserves the plan's spine — opaque monotonic client-held cursors with epoch inside, at-least-once + idempotent receiver, reset as the universal pressure valve, `synced ⊕ pending` re-derivation, shared fan-out per `(name, args)`. What changed from v1, in one place:
...
- **The freshness surface is on the wire**: `live` frames disclose `freshness` + `updatedAt`, visibility is connection metadata, and the consumer surface speaks the locked vocabulary (`subscription.state`, `sub.freshness`, `connection.ensure()`).
...
**Channel args are wire-plain by construction.** Schema revival happens above the address: args serialize as plain JSON, and no schema-revived type (a Date, a custom value type) ever participates in address derivation. This is stated, not assumed — it is what makes two independent implementations derive identical addresses *(conformance: `address-vectors`)*.
...
**`auth { token }`** — mid-connection credential refresh. Subscriptions, cursors, and in-flight calls are untouched (Centrifugo refresh lineage: positions survive re-auth). Server replies `auth { ok: true, expiresIn }` or `auth { ok: false, code }` then closes 4101. Two clauses harden it:
...
**The snapshot carve-out.** A channel mid-snapshot folds the transaction's effects into the snapshot: the snapshot is taken at a position at or past the transaction, so its effects arrive inside it and no delta message for that channel exists. A transaction spanning a live channel and a mid-snapshot channel is therefore atomic per boundary — the live channel's frame and the snapshot's commit are each atomic apply units — and the cross-boundary tear is bounded by the snapshot bracket, healed at its `live`.

**Resume.** At resume the server **regroups tail frames by transaction** across the channels in the resume batch: tail entries that share a transaction (the changelog envelope carries the identity) pack into one frame per socket, per-request computation against the resume batch, no per-client state — the stateless-node invariant survives *(conformance: `atomicity-resume-regroup`)*.
...
0. **High-water rejection.** A frame whose cursor is at or below the channel's applied cursor is a no-op. Redelivery (gap re-subscribe, at-least-once overlap) becomes safe by position, not by value — membership transitions (enter/leave) are not value-idempotent, so position is the mechanism at-least-once actually needs.
1. **Cursor advances only on apply, never on receive.** A queued or buffered frame never advances the stored cursor, and the persisted cursor is always consistent with the persisted snapshot — a crash between receive and apply must resume at the last **applied** position, or the client silently skips data it never rendered *(conformance: `resume-cursor-on-apply`)*.
2. **Queued frames discard at snapshot commit and at reset.** The snapshot subsumes them; a stale queued frame applied after a resnapshot writes older values over newer ones *(conformance: `resume-queued-discard`)*.
...
6. **`cleared` writes `undefined`, never deletes.** Removed keys slip the renderer's snapshot diff by design — which is why the wire says `cleared` instead of carrying delete semantics.
...
```
limits { maxMessageBytes, maxBatchFrames }
```

The client sizes outbox batches under `maxBatchFrames`, and refuses an oversized call locally with a teaching error before it ever leaves (the content survives in the outbox — the hard-fail conversion rule). Server-side valves — `maxBufferedBytes` (reference default 16MB), log-collapse thresholds, per-connection inbound frame caps, channels-per-connection — stay **unadvertised settings**: they are operator knobs, not client contracts, and advertising them buys nothing a reason code doesn't. All constants are settings with reference defaults; the hobbyist configuration omits `limits` entirely.
...
Codes are terminal-state instructions, not prose: 4001 / 4100-without-auth-callback → `failed`; 4101 with an auth callback → `reconnecting` with a fresh token, without one → `failed`; 4103 → fresh handshake; 4400 → do not fight (§3); 4500/4502/4504 → normal backoff; 4501 → redirect; 4503 → `retryAfter`. The settle-class mapping — which codes end a write as retry-later and which as discard-forever — is §6's table. Code-table conformance vectors pin every row, including that `4204` has no legal answer under v1's table and that `4203` relocates the config error off `4202`, which v2 reserves for denial *(conformance: `codes-table-vectors`)*.
...
**The result cache is core, not a capability.** The ledger retains result payloads for their TTL, and a duplicate `call` id replays the original `result` instead of re-executing — universal retry-safety, for reconnect replays and manual action retries alike, its cost already paid by the durable ledger *(conformance: `ledger-result-replay`)*. v1 staged this as a `result-cache` capability; v2 promotes it to the mandatory core.
...
**Failure outcomes survive the trim.** Rejected-call outcomes (id, code, message) are retained for the failure TTL and **cross-checked at reconnect trim**: when `welcome.lastCallID` trims an outbox entry the client never saw settle, the client asks the retained outcomes before treating it as applied — a rejected-then-trimmed call surfaces its error and settles as a rejection-park (§6) instead of silently reverting committed-looking work *(conformance: `ledger-trimmed-outcome-crosscheck`)*.

**TTL policy is settings**, each answering a constraint: `ledger.watermarkTTL` must exceed any horizon at which a client may still replay (reference default `'30d'`); `ledger.failureTTL` must be at least the park expiry horizon so a returning client inside the envelope can still learn its rejection (reference default `'7d'`, the horizon's value); `ledger.resultTTL` bounds success-payload retention (reference default `'24h'`). Unknown clientID (watermark expired or never existed) → `lastCallID: 0`, replays accepted — the residual double-apply window is vanishingly small against a multi-week watermark TTL, the cheap analog of Replicache's ClientStateNotFound posture.
...
1. **Socket opens** → `hello { protocol, clientID, auth }`, then `visibility` re-announces if hidden.
...
5. **Outbox replay, immediately after** — not gated on sub completion. One array frame of remaining `call`s in seq order — **gated by age**: entries inside the blip window replay blind; aged entries are withheld from this frame entirely, held for verify at their channels' `live`, and only then sent (released) or parked, per the outcome vocabulary (§6). Fresh replay racing sub replay is by design: `synced ⊕ pending` re-derives regardless of order.
...
`retention: 'none'` answers every resume with a snapshot **directly** — no `reset` round trip, the cursor-expired path that already exists *(conformance: `retention-none-snapshot`)*. The scenario's own math prefers it on the dominant per-doc channel (a 2-minute absence ≈ 700KB of tail vs a 150-200KB snapshot): hot per-doc channels hold near-zero retention and snapshot at resume, and the `sub`-time bytes rule (§2) picks per resume where a log exists.
...
Defined once, consumed twice: by park-then-verify (the aged-replay machine — its gates capture evidence at optimistic apply, withhold aged entries from the resend, verify at `live`) and by the auth-rejection class (permission revoked while away — the permissions corpus's conflict/park joint). One vocabulary, so a rejected write settles identically whether the rejector was a validation rule, a permission gate, or a verify mismatch.
...
**The resend-gate states map onto the vocabulary**: an outbox entry is **sent** (fresh, inside the blip window, or released by verify), **withheld** (aged, awaiting verify at `live` — never offered to the server, across any number of welcomes), or **parked** (one of the three classes above — never sent; release re-mints under a fresh id per the watermark rule, §4).
...
```js
const sub = subscribe('invoices.byId', () => ({ id }))
...
`state` runs `loading` (no `live` yet — initial data incomplete), `stale` (disconnected past the 5s grace, `reset` pending, or behind the declared freshness), `current` (at the stream head as of the last `live`), and `denied` (confirmed 4202 after retry-then-prune, §6). The freshness disclosure is what keeps priced staleness honest: a `'manual'` dashboard renders its `updatedAt` age, and the affordance is a named deliverable, not an implied one. A derived per-channel pending count stays bench-deferred (§9, Q5).
...
**Version-skew policy.** The reference server speaks **N and N−1** — one integer back. Clients send their highest, `welcome` downgrades, and **the server deploys first**: incorrect deploy order is the named downtime cause, and the expand/contract deploy-order quick-reference is a reference-server doc page.
...
| 7 | Batch frame limits | **Resolved, §2 Limits** — `welcome.limits` advertises `maxMessageBytes` + `maxBatchFrames` (client-actionable only), server valves stay unadvertised settings, 4204 minted. |
...
| 10 | Conformance suite shape | **The case catalog exists** — [`ws-protocol-conformance.md`](ws-protocol-conformance.md) lists every case this spec cites with its gate mapping. The harness decision (recorded wire transcripts vs live driver) is co-authored with the maintainer and recorded there at ratification. |
