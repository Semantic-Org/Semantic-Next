...
* **Enhancement** - Configured vocabulary functions (`humanize`, `toTitleCase`, `getArticle`, `toBoolean`) attach their config as one tree-shakable expression, so a bundle only pays for the vocabularies it imports — the attribute-codec path that pulls in string casing no longer carries any of them
