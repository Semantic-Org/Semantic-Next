...
* **Enhancement** - `formatDuration` prints the clock form under `{ mixed: true }`, whole units and the remainder in the next: `390000` is `'6:30'` where the default reads `'6.5m'`, an hour and five minutes is `'1:05:00'`, and below a minute it is `'49s'`, rounded to the second and never a zero for a positive value. `formatDuration.config.mixed` sets it once at boot. It is the one print that does not read back through `toDuration`
