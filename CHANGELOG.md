...
* **Enhancement** - The `createErrors` production line's verb is a free slot — a stable token position carrying the layer's own classification of the line. Any word passed as `verb` in `error()`/`throwError()` options or to `error.line(code, at, { verb })` renders in the line; `refused` stays the default when none is given, so existing lines are unchanged. The published parse pattern widens to `/^(\S+) (\S+) \[([\w-]+)\] (.*)$/m`
