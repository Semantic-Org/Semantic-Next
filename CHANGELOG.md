...
* **Enhancement** - The `createErrors` production line's verb is now class-bearing: `refused` (a rule declined the action), `caught` (your code threw and the layer reports the seat), or `noted` (an observation, informational). Pass `verb` in `error()`/`throwError()` options or to `error.line(code, at, { verb })` — `refused` stays the default and a verb outside the vocabulary falls back to it, so existing lines are unchanged. The published parse pattern widens to `/^(\S+) (refused|caught|noted) \[([\w-]+)\] (.*)$/m`
