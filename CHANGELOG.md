...
* **Feature** - Added `ReactiveObject` / `reactiveObject(initial, options)` — fine-grained reactivity over a plain object at the granularity of a PATH (dotted keys, positional `[i]` indices, keyed `[#id]` array segments). A reader of one path wakes only when that path's value changes, where a `Signal` holding an object wakes every reader on any change. Carries `get`/`peek`/`set`/`remove`/`replace`/`clear`, equality-gated writes that also wake overlapping ancestor and descendant readers, deep-reader reseed on bulk inbound `replace`, and `prune`/`stop` teardown. Shares Signal's `safety`/`equality`/`clone` configuration
* **Feature** - `ReactiveObject` gains the Signal-parity surface — `depend(path)` (subscribe to a path without reading it), `has(path)` (tracked existence that tells a stored `undefined` from an absent key), `notify(path)` (force-wake a path and every descendant after an in-place mutation), `raw(path?)` (the live reference, never cloned even under `safety: 'clone'`), and `clone(path)` (a tracked, detached copy) — plus a monotonic `version` counter seedable via the `version` option. `remove(path)` now keys on presence, so a key holding a stored `undefined` is removable
...
* **Feature** - `timezones` is exported on its own: the shorthand alias table `formatDate` reads, the same object as `formatDate.config.timezones`, so a package that only needs the table imports it without the formatter
* **Feature** - Added [`editDistance`](https://next.semantic-ui.com/docs/api/utils/strings#editdistance) — single-character edits between two strings (Levenshtein), Unicode-aware by default (NFC-normalized, astral characters count as one edit, grapheme clusters via `grapheme: true`). `max` caps the search and returns `Infinity` once the distance provably exceeds it, so candidate filtering stays cheap. `swaps: true` counts an adjacent swap as one edit (the restricted Damerau-Levenshtein reading), `ignoreCase` folds case, and per-operation costs (`insertCost`/`deleteCost`/`replaceCost`/`swapCost`) express the weighted variants. Short default-option comparisons run bit-parallel (Myers), everything else through one banded DP engine
* **Feature** - Added [`similarity`](https://next.semantic-ui.com/docs/api/utils/strings#similarity) — how alike two strings read, 0..1, one minus the edit distance over the longer length. `min` is a score floor that stops a comparison early and reports 0, the cheap-rejection shape for fuzzy-match loops. Takes the same text options as `editDistance`
* **Feature** - Added [`suggest`](https://next.semantic-ui.com/docs/api/utils/strings#suggest) — the nearest candidate to a mistyped word, or `null` when nothing reads close enough, for did-you-mean teaching messages. Returns the candidate verbatim, never prose, so the caller owns the sentence. Candidates read generously (an array or `Set` of names, a lone name, or the keys of a plain object or `Map`), `threshold` (default 0.6 similarity) scales the bar with word length, and case differences and adjacent swaps read as typos by default. Ties keep the earlier candidate. `count` returns the plausible candidates as an array instead, best-first, up to `count` of them (`[]` when none) — same confidence gate; to rank everything under an explicit edit cap, filter with `editDistance` and `max`
* **Feature** - Added [`fill`](https://next.semantic-ui.com/docs/api/utils/strings#fill) — bracketed placeholders in a string filled from a values object, the shape a copy table takes, where a whole sentence stays one editable string and only its variable parts are named. Values resolve from the call's object first and a `defaults` object second, and a key that resolves to nothing leaves its placeholder in place, so a typo in a copy table shows itself in the UI instead of shipping a blank sentence (`missing` chooses otherwise, as a string or a `(key, placeholder)` function). Keys read the path grammar, so `{user.name}` reaches into a nested object and an array of values reads positionally. Substitution runs once and is never rescanned, so a filled value carrying `{other}` cannot reach a second key. A value a string cannot faithfully hold (a plain object, a `Date`, `NaN`) reads as missing rather than as `[object Object]`, and `transform` is where an app says how those print. `open` and `close` read copy that already speaks another grammar (`${name}`, `%{name}`)
* **Feature** - `hashCode`, `generateId`, `isValidId`, and `parseId` take `ignoreConfig` to resolve as if no global `fn.config` were set (call options > preset only) — deterministic output for library code regardless of host-app defaults. `memoize`'s default key hash now pins it, so an app-wide `hashCode.config` can no longer change how memoize keys
* **Breaking** - `fatal` has been removed — its construction half is `createErrors`' coded builders, and its async raise is `error()`'s report channel. Callers that reported and continued migrate to `error(code, at, { explanation })`; callers that meant to stop migrate to `throwError(...)`, which throws synchronously (several internal callsites relied on the async raise and fell through to a second accidental error — those are fixed by the migration)
* **Feature** - Added coded errors — top-level [`error`](https://next.semantic-ui.com/docs/api/utils/debug#error) and [`throwError`](https://next.semantic-ui.com/docs/api/utils/debug#throwerror) with a development/production message split, and [`createErrors`](https://next.semantic-ui.com/docs/api/utils/debug#createerrors) binding them per namespace (mirroring `createLogger` over `log` — the factory pre-fills, callsite options win). `namespace` and `ErrorClass` ride the options bag per call. Development carries the teaching `explanation`, production carries one uniform greppable line `<namespace> refused [<code>] <at>` (also available alone as `error.line(code, at)`). `error()` reports through the error channel asynchronously (routing to `globalThis.onError` when installed) and returns the Error; `throwError()` throws. Codes ride `error.code` for programmatic routing, structured data rides `error.detail`. Explanations are written to fold out of production builds at the callsite (`explanation: isDevelopment ? '...' : 0`) — a bundler define folds branches, never arguments
* **Enhancement** - Coded errors from [`createErrors`](https://next.semantic-ui.com/docs/api/utils/debug#createerrors) now carry their address as `error.at` alongside `code` and `detail`, and a development message leads with the production line before the explanation — the address prints in every posture and explanations never need to restate it
* **Feature** - Added [`createLogger`](https://next.semantic-ui.com/docs/api/utils/debug#createlogger) — binds `log` to shared defaults (`namespace`, `timestamp`, any `log` option) and returns the flat bundle `{ log, debug, info, warn, error }` for destructuring. The level wrappers absorb the level slot only (`info(message, options)`), the bound `log` keeps it, and callsite options shallow-merge over the factory defaults. Every member has a once form, `warnOnce(key, message = key, options)` with `debugOnce`, `infoOnce`, `errorOnce`, and `logOnce` (level slot kept): the first call with a key prints what the plain member prints, every later call with that key on the same logger is silent whatever the message, one memory serves the whole family, and a fresh logger is the clean slate. The key names the condition, never the text, and development advice keeps `isDevelopment &&` at the callsite so a bundler folds the call and its message together. Both debug families export an `error`, so a file binding both aliases at the destructure: `const { error: logError } = createLogger(...)` beside `createErrors`' `error`
* **Enhancement** - `log`'s default title now prints the namespace verbatim — `db` shows as `db`, not `Db` (was title-cased)
...
* **Feature** - `detectChanges` takes three more options. `equality` swaps the leaf comparator (defaults to `isEqual`, the same option `trackWrites` takes) for diffing loosely-typed data. `ignoreKeys` drops key names at any depth so a volatile or local-only field never reaches the changeset. `collapseKeys` diffs a key as one whole value without descending — the same leaf treatment a `Map`/`Date` gets, for a subtree whose own keys are dynamic strings that would mis-parse as wire paths (a map keyed by `contacts[#id].field`). See [`detectChanges`](https://next.semantic-ui.com/docs/api/utils/objects#detectchanges)
* **Enhancement** - `trackWrites` now forwards its `equality` option into the snapshot path diff, so a custom comparator is honored on the default `returnPaths` path. Previously it only applied to the `returnPaths: false` fast path and exotic snapshots, so the same call could answer "did it change" differently depending on `returnPaths`. See [`trackWrites`](https://next.semantic-ui.com/docs/api/utils/objects#trackwrites)
* **Feature** - `trackWrites` now id-addresses keyed arrays in its reported paths by default (`todos[#id].complete` instead of `todos.0.complete`), so editing a field on every record of a collection reads back as per-record writes that survive a reorder, with zero config. Keyed paths come from the snapshot diff, so requesting them forces the snapshot strategy even on a large value; an explicit `strategy: 'proxy'` (or `onWrite`) keeps the no-clone proxy whose paths stay positional. `keyed: false` opts out, `fields` overrides the identity fields. See [`trackWrites`](https://next.semantic-ui.com/docs/api/utils/objects#trackwrites)
* **Feature** - `get`, `set`, and `unset` now understand a keyed `field[#id]` path segment that addresses an array element by identity instead of position — `get` returns the element or undefined, `set` replaces a present key or appends an absent one (a field write through an absent key is a no-op), `unset` splices the element out. Each takes a trailing `fields` argument to override the identity fields. The `field[#id]` paths from keyed `detectChanges` apply back through them directly, surviving a reorder
* **Feature** - Added [`keyedPath`](https://next.semantic-ui.com/docs/api/utils/paths#keyedpath) — the keyed spelling of a positional path, rewriting each positional array segment (`items.0.qty`, `items[1]`) to its `[#id]` form where the addressed element carries a path-safe identity, resolved against the object at call time. A positional address is only stable while the array doesn't move, the keyed address survives a reorder — the same identity keyed `detectChanges` emits. Keyless arrays, unresolvable paths, and already-keyed spellings pass through, and it returns the input string itself when nothing rewrites so callers compare by reference. `fields` overrides the identity fields
* **Feature** - A keyed `[#id]` segment carries any id without `]` — emails (`users[#jack@semantic-ui.com]`), dotted and compound ids (`items[#200.40.50]`) — across `get`/`set`/`has`/`unset`, `keyedPath`, `detectChanges`, `trackWrites`, `trackReads`, and `ReactiveObject` paths. A dot inside a bracket belongs to the id, never the path, and an id carrying a prototype word (`[#mail.constructor.dev]`) is identity, not a climb — the pollution guard reads path structure only
* **Feature** - Added [`eachPath`](https://next.semantic-ui.com/docs/api/utils/paths#eachpath) — walk the paths a path passes through, shortest first (`todos[#a].done` visits `todos`, `todos[#a]`, `todos[#a].done`). Every visited value is itself resolvable through `get`, a bracket splits its segment into container and element, and `self: false` visits only the ancestors above the target — the walk a subsumption check or invalidation pass needs. Returning `false` breaks early, like `each`
* **Feature** - Added [`parsePath`](https://next.semantic-ui.com/docs/api/utils/paths#parsepath) and [`pathFrom`](https://next.semantic-ui.com/docs/api/utils/paths#pathfrom) — the semantic reading of a path and its inverse. `parsePath` returns segments (`{ type: 'field', name }`, `{ type: 'key', key }`, `{ type: 'index', index }`, `{ type: 'wildcard' }`), `null` for a path that doesn't parse, cached and bounded via `parsePath.config`. `pathFrom` builds the path back from segments, plain segment strings, or a mix; positional indexes emit the dot form, so a round-trip normalizes `items[2]` to `items.2`. [`splitPath`](https://next.semantic-ui.com/docs/api/utils/paths#splitpath) ships the textual reading beside it — the segment split where a dot inside a bracket belongs to the key
* **Feature** - Added [`pathKey`](https://next.semantic-ui.com/docs/api/utils/paths#pathkey), [`isPathKey`](https://next.semantic-ui.com/docs/api/utils/paths#ispathkey), and [`elementPath`](https://next.semantic-ui.com/docs/api/utils/paths#elementpath) — the emission side of the grammar. `pathKey` is an element's key as it can appear in a path, `null` when unkeyed or carrying `]` — the item-to-path route, where `elementKey` returns the raw identity. `isPathKey` guards a key that exists before any item does, like a user-typed id at a form boundary. `elementPath` builds the path of one element under a list path — `{ key }` in the `[#key]` form, `{ index }` in the dot form — and throws rather than emit a path nothing can parse
* **Feature** - Added [`pathCovers`](https://next.semantic-ui.com/docs/api/utils/paths#pathcovers), [`pathsOverlap`](https://next.semantic-ui.com/docs/api/utils/paths#pathsoverlap), and [`wildcardPath`](https://next.semantic-ui.com/docs/api/utils/paths#wildcardpath) — the segment-aligned relations over the grammar. `'items'` covers `'items[#r7].amount'` and never `'itemsLog'`, a `'*'` matches any single segment (`'lines.*.cost'` covers `'lines[#a].cost'`), and keyed and positional indexes stay apart — `[#7]` never means `[7]`. `wildcardPath` is the wildcard spelling of a concrete path (`lines[#a].tax` -> `lines.*.tax`), the same address at list grain, so per-element paths register under one spelling
* **Feature** - Added [`expandPath`](https://next.semantic-ui.com/docs/api/utils/paths#expandpath) — expand a path spelling into the concrete paths it addresses, always as an array. A leading-dot relative spelling resolves against `{ from }` (`'..tax'` from `'lines[#a].qty'`, a keyed segment counting one level like any other), and a `'*'` enumerates the array at that level of `{ doc }`, keyed elements taking the `[#key]` spelling so a derived write respects a keyed override. Both spellings compose in one call
* **Breaking** - `generateId` now takes an options object instead of a seed. It defaults to a sortable 26-char [ULID](https://github.com/ulid/spec) and takes `usage` presets (`db`, `page`, `link`, `token`, `secret`, `code`) that carry the consensus length, alphabet, and structure per channel, plus `length`, `bits`, `prefix`, `checksum`, `upper`, `format: 'uuid'`, `group`, and a global `generateId.config`. The `page` preset is letter-first so its ids are valid CSS identifiers — the old base-36 output could lead with a digit. `code` is the read-aloud tier: short, uppercase, checksummed, and grouped for license and redemption codes. `secret` is the credential tier for session secrets and the tokens derived from them: 256 bits over 53 checksummed characters, clear of every current floor, so an app never spells the number itself. `bits` is that same width dial in the unit a guessing budget is argued in, and sizes any preset. See [`generateId`](https://next.semantic-ui.com/docs/api/utils/crypto#generateid)
* **Breaking** - Renamed `generateID` / `isValidID` / `parseID` to `generateId` / `isValidId` / `parseId`, aligning trailing-`ID` casing with the JS ecosystem convention (`getElementById`, `userId`)
* **Feature** - Added [`isValidId`](https://next.semantic-ui.com/docs/api/utils/crypto#isvalidid) and [`parseId`](https://next.semantic-ui.com/docs/api/utils/crypto#parseid) — the offline inverse of `generateId`. `isValidId` is the cheap rejection before a lookup (prefix, length, alphabet, checksum, reading loose for case/`I``L``O`/hyphens), `parseId` splits an id into `{ prefix, body, checksum, timestamp }`, decoding the ULID clock for the `db` preset
* **Breaking** - `hashCode` now takes `usage` presets that mirror `generateId`: `hash` (the default 53-bit number for in-heap keys), `content` (a 64-bit Crockford string), `fingerprint` (a 128-bit string for dedup), and `secure` (an async SHA-256 string). A `format` override (`crockford`, `hex`, `number`) and a global `hashCode.config` complete the shape. The default call still returns the same number, so existing callers are unaffected. Replaces the old `prettify` boolean. The earlier `fast` option and UMASH path are gone. See [`hashCode`](https://next.semantic-ui.com/docs/api/utils/crypto#hashcode)
* **Breaking** - Removed `prettifyHash`. Its only role was backing `hashCode`'s `prettify` option, now superseded by `hashCode(value, { usage: 'content' })` for a readable Crockford string
* **Breaking** - `getRandomSeed` dropped its `Math.random` fallback — it now always uses `crypto.getRandomValues`
* **Feature** - Added [`trackWrites`](https://next.semantic-ui.com/docs/api/utils/objects#trackwrites) — runs a callback against a value and reports whether it changed it, with the changed fields as dot paths resolvable via `get()`. Snapshots small values so the callback sees the real object, write-tracks large ones through a proxy so cost scales with writes instead of value size, with a `strategy` option to pin either path, `returnPaths: false` for hot paths, and `onWrite` for streaming per-write key paths
* **Feature** - Added [`trackReads`](https://next.semantic-ui.com/docs/api/utils/objects#trackreads) — the read companion to `trackWrites`: runs a callback against a value and reports which paths it read, for collecting a computed's dependencies, deriving a memo key, or auditing access. Returns `reads` (value paths, pair with `detectChanges` changed) apart from `structure` (a container's shape — an array's length, iteration, or `Object.keys`, pair with added/removed), so a value-only dependency set doesn't silently miss array growth. Proxy-only and read-only — a write through the tracked wrapper throws, so the input is never mutated. Keyed `field[#id]` paths by default, `keyed: false` for positional, `returnPaths: false` plus `onRead` for streaming
* **Feature** - Added [`detectChanges`](https://next.semantic-ui.com/docs/api/utils/objects#detectchanges) — structural diff between two values reported as added, removed, and changed dot paths. Also powers `trackWrites` path reporting on the snapshot side
* **Feature** - Added [`set`](https://next.semantic-ui.com/docs/api/utils/paths#set) — write twin of `get` for string paths, creating missing intermediates (arrays for numeric segments) and refusing prototype-climbing segments. Paths from `trackWrites` and `detectChanges` apply back directly
* **Feature** - Added [`unset`](https://next.semantic-ui.com/docs/api/utils/paths#unset) — delete twin of `get`/`set` for string paths. Applies `detectChanges` removed paths, leaves holes at array indices so sibling paths stay valid across multiple removals
* **Feature** - Added [`has`](https://next.semantic-ui.com/docs/api/utils/paths#has), the existence twin of `get`. Returns true when a string path resolves to a real location, even one holding `undefined`, which `get` reports the same as a missing path. Shares `get`'s resolver and full path grammar (dotted keys, `[0]` indices, `[#id]` keyed segments, literal dotted-key fallbacks)
...
* **Feature** - Added [`formatDuration`](https://next.semantic-ui.com/docs/api/utils/dates#formatduration), the inverse of `toDuration`. Prints milliseconds the way a person reads them (`242100` as `'4m 2s'`, `300000` as `'5m'`), reads anything `toDuration` reads, and its decimal form (`format: 'decimal'`, `90000` as `'1.5m'`) reads back through `toDuration` string for string, so a config can take `'5m'` on the way in and show `'5m'` on the way out. For the decimal form `decimals` is a maximum, `unit` holds one unit down a column, `separator` goes between the number and the unit (`'1.5 minutes'`), and `lossless` picks the largest unit that reads back to exactly the same value (`100000` is `'100s'`, not `'1.7m'`), the print a config or debug view wants. The unit ladder is editable at boot via `formatDuration.config.units`, spelled in `toDuration`'s vocabulary
* **Enhancement** - `formatDuration` takes a `format`, the word `formatDate` already takes: `'units'` is the default and prints the two largest non-zero whole units (`'6m 30s'`, `'1h 5m'`), `'clock'` prints whole units and the remainder in the next (`390000` is `'6:30'` where the default reads `'6.5m'`, an hour and five minutes `'1:05:00'`), and `'decimal'` prints one unit with decimals (`'6.5m'`). The first two read `'49s'` below a minute, round to the second, and never print zero for a positive value. `formatDuration.config.format` sets the form once at boot. Only the decimal form reads back through `toDuration`, and a word outside the three answers `null`
...
* **Feature** - [Reading values](https://next.semantic-ui.com/docs/api/dates/reading-values) is strict by default, ISO 8601 with the way out in every refusal, and `{ loose: true }` on every factory reads whatever `Date` reads and gives null for the rest. `configure({ dayFirst: true })` reads `07.09.2026` as the 7th of September, and a count written as text keeps its unit, so `days('30')` from a form is thirty days
* **Feature** - [Zones](https://next.semantic-ui.com/docs/api/dates/zones) answer to IANA names, cities like `'Berlin'` and `'los angeles'`, abbreviations, offsets and `configure({ zoneAliases })`. A zone is a view and the wire form is the instant in UTC, adding days across a daylight saving change keeps the wall clock, and adding hours counts hours
* **Feature** - [Ranges](https://next.semantic-ui.com/docs/api/dates/range-bounds) are named by what they hold. A date range runs through its end, a datetime or time range until it, and a time range crosses midnight, so `timeRange('22:00', '06:00')` is the night shift. `points(step)` walks a range and `split(step)` cuts it, and a date does both over its own day: `today().split('hour')`
* **Feature** - [Durations](https://next.semantic-ui.com/docs/api/dates/durations) keep their fields as written until `balance()`, which carries a length to hours and never invents a calendar day from clock time, and a duration from `until()` remembers its anchor so months and years total. `format()` prints a stored count of milliseconds as hours and minutes
* **Feature** - [Helpers](https://next.semantic-ui.com/docs/api/dates/helpers): `compare`, `earliest`, `latest`, `kindOf`, `weekday`, `weekdayNames`, `monthNames`, `startOfToday`, `endOfToday`, the `is*` guards and the brand symbols for recognising a value without bundling the classes. Every refusal is a coded `RangeError` or `TypeError`, listed on the [errors](https://next.semantic-ui.com/docs/api/dates/errors) page
