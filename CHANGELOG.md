...
* **Enhancement** - `formatDuration` takes a `format`, the word `formatDate` already takes: `'decimal'` is the shipped form and the default, `'clock'` prints whole units and the remainder in the next (`390000` is `'6:30'` where the default reads `'6.5m'`, an hour and five minutes `'1:05:00'`), and `'units'` prints the two largest non-zero whole units (`'6m 30s'`, `'1h 5m'`). Both read `'49s'` below a minute, round to the second, and never print zero for a positive value. `formatDuration.config.format` sets the form once at boot. The two new forms do not read back through `toDuration`, and a word outside the three answers `null`
