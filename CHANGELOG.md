...
* **Feature** - Added [`formatDuration`](https://next.semantic-ui.com/docs/api/utils/dates#formatduration), the inverse of `toDuration`. Prints milliseconds in the largest unit filled (`300000` as `'5m'`, `90000` as `'1.5m'`), reads anything `toDuration` reads, and every string it prints reads back through `toDuration`, so a config can take `'5m'` on the way in and show `'5m'` on the way out. `decimals` is a maximum, `unit` holds one unit down a column, and `lossless` picks the largest unit that reads back to exactly the same value (`100000` is `'100s'`, not `'1.7m'`), the print a config or debug view wants. The unit ladder is editable at boot via `formatDuration.config.units`, spelled in `toDuration`'s vocabulary
