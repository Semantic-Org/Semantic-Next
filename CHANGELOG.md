...
### Utils
* **Breaking** - `generateID` now takes an options object instead of a seed. It defaults to a sortable 26-char [ULID](https://github.com/ulid/spec) and takes `usage` presets (`db`, `page`, `slug`, `token`) that carry the consensus length, alphabet, and structure per channel, plus `length`, `prefix`, `checksum`, `format: 'uuid'`, `group`, and a global `generateID.config`. The `page` preset is letter-first so its ids are valid CSS identifiers — the old base-36 output could lead with a digit. See [`generateID`](https://next.semantic-ui.com/docs/api/utils/crypto#generateid)
...
* **Breaking** - `hashCode` now uses a single deterministic 53-bit hash (cyrb53), dropping the `fast` option and the UMASH path. Same call shape, wider hash, no behavior change for default callers
