...
* **Breaking** - `generateId` now takes an options object instead of a seed. It defaults to a sortable 26-char [ULID](https://github.com/ulid/spec) and takes `usage` presets (`db`, `page`, `link`, `token`, `code`) that carry the consensus length, alphabet, and structure per channel, plus `length`, `prefix`, `checksum`, `upper`, `format: 'uuid'`, `group`, and a global `generateId.config`. The `page` preset is letter-first so its ids are valid CSS identifiers — the old base-36 output could lead with a digit. `code` is the read-aloud tier: short, uppercase, checksummed, and grouped for license and redemption codes. See [`generateId`](https://next.semantic-ui.com/docs/api/utils/crypto#generateid)
