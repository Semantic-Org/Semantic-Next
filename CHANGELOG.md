...
* **Feature** - Added [`pathCovers`](https://next.semantic-ui.com/docs/api/utils/paths#pathcovers), [`pathsOverlap`](https://next.semantic-ui.com/docs/api/utils/paths#pathsoverlap), and [`patternFrom`](https://next.semantic-ui.com/docs/api/utils/paths#patternfrom) — the segment-aligned relations over the grammar. `'items'` covers `'items[#r7].amount'` and never `'itemsLog'`, a `'*'` matches any single segment (`'lines.*.cost'` covers `'lines[#a].cost'`), and keyed and positional indexes stay apart — `[#7]` never means `[7]`. `patternFrom` collapses a concrete path to its dependency-pattern spelling (`lines[#a].tax` -> `lines.*.tax`)
