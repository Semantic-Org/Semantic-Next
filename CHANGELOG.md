...
* **Bug** - `error()` from `createErrors` now falls back to `console.error` when no `globalThis.onError` handler is installed — the deferred report previously threw as an unhandled error, crashing apps that never installed a handler
