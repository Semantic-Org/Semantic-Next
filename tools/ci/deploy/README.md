...
heap 🧠): same GitHub App + avatar identity, same sticky single comment.
...
A deploy runs only when a maintainer adds the matching label — the gate that
keeps every PR push from spending build minutes, and keeps the spend a human
decision.
...
`⚪ NA` is a target this PR didn't request. While a deploy is in flight the table
shows start time, an ETA, and the expected build time (median of recent deploys,
hidden until a few runs seed it). All times are US Eastern.
...
- **`pr-deploy.yml`** (on a labeled `pull_request`)
  - `announce` posts the Building comment. It holds the bot token but runs no PR
    code — it renders from `main`'s reporter against the event, and estimates the
    ETA from recent run durations via the Actions API.
  - `docs` / `mcp` each deploy their target with the Vercel token and
...
The suite's rule holds: the job that runs PR code (`docs` / `mcp`) never holds
...
deploy. On the PR that first adds this harness, `main` doesn't have the tooling
or the bot secrets yet, so the comment jobs stay inert until it lands and the App
is set up — the deploy jobs still run, a self-test.
...
| `reporter.js` | CLI: facts (or in-flight timing) → `comment.md` + `preview-report.json` |
