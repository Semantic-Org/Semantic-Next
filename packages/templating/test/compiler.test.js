...
describe('TemplateCompiler', () => {
  ...
  describe('define blocks', () => {
    it('should compile a basic define block', () => {
      const compiler = new TemplateCompiler();
      const template = `
        {{#define getUsers() as users}}
    ...
    it('should compile a define block using load alias', () => {
      const compiler = new TemplateCompiler();
      const template = `
        {{#load getData() as data}}
          <div>{{data.name}}</div>
        {{/load}}
      `;
      const ast = compiler.compile(template);
      expect(ast[0].type).toBe('define');
      expect(ast[0].expression).toBe('getData()');
      expect(ast[0].alias).toBe('data');
    });

    it('should compile a define block without alias (this reference)', () => {
      const compiler = new TemplateCompiler();
      const template = `
        {{#define getUsers()}}
          <div>{{this.length}} users</div>
        {{/define}}
      `;
      const ast = compiler.compile(template);
      expect(ast[0].type).toBe('define');
      expect(ast[0].expression).toBe('getUsers()');
      expect(ast[0].alias).toBeNull();
    });

    it('should compile a define block with destructuring', () => {
      const compiler = new TemplateCompiler();
      const template = `
        {{#define getUser() as { name, email, ...rest }}}
          <div>{{name}} - {{email}}</div>
        {{/define}}
      `;
      const ast = compiler.compile(template);
      expect(ast[0].type).toBe('define');
      expect(ast[0].expression).toBe('getUser()');
      expect(ast[0].alias).toBeNull();
      expect(ast[0].destructuring).toEqual({
        properties: ['name', 'email'],
        rest: 'rest'
      });
    });

    it('should compile a define block with loading state', () => {
      const compiler = new TemplateCompiler();
      const template = `
        {{#define getUsers() as users}}
          <div>{{users.length}} users</div>
      ...
      const ast = compiler.compile(template);
      expect(ast[0].type).toBe('define');
      expect(ast[0].pending).toBeDefined();
      expect(ast[0].pending.type).toBe('pending');
      expect(ast[0].pending.content).toEqual([
        { type: 'html', html: '\n          <div>Loading users...</div>\n        ' }
      ]);
    });

    it('should compile a define block with before state (alias for loading)', () => {
      const compiler = new TemplateCompiler();
      const template = `
        {{#define fetchData() as data}}
          <div>{{data.result}}</div>
        {{before}}
    ...
    it('should compile a define block with error state', () => {
      const compiler = new TemplateCompiler();
      const template = `
        {{#define getUsers() as users}}
          <div>{{users.length}} users</div>
      ...
      expect(ast[0].error.type).toBe('error');
      expect(ast[0].errorAlias).toBe('error');
      expect(ast[0].error.content).toEqual([
        { type: 'html', html: '\n          <div>Error loading users: ' },
        { type: 'expression', value: 'error.message' },
        { type: 'html', html: '</div>\n        ' }
      ]);
    });

    it('should compile a define block with custom error alias', () => {
      const compiler = new TemplateCompiler();
      const template = `
        {{#define getUsers() as users}}
          <div>{{users.length}} users</div>
      ...
      expect(ast[0].type).toBe('define');
      expect(ast[0].error).toBeDefined();
      expect(ast[0].errorAlias).toBe('e');
    });

    it('should compile a define block with else state', () => {
      const compiler = new TemplateCompiler();
      const template = `
        {{#define getUsers() as users}}
          <div>{{users.length}} users</div>
      ...
      expect(ast[0].else).toBeDefined();
      expect(ast[0].else.type).toBe('else');
      expect(ast[0].else.content).toEqual([
        { type: 'html', html: '\n          <div>No users found</div>\n        ' }
      ]);
    });

    it('should compile a complete define block with all clauses', () => {
      const compiler = new TemplateCompiler();
      const template = `
        {{#define getUsers() as users}}
          <div>
        ...
        {{error as err}}
          <div class="error">Failed: {{err.message}}</div>
        {{else}}
      ...
      expect(ast[0].type).toBe('define');
      expect(ast[0].expression).toBe('getUsers()');
      expect(ast[0].alias).toBe('users');
      expect(ast[0].pending).toBeDefined();
      expect(ast[0].error).toBeDefined();
      expect(ast[0].errorAlias).toBe('err');
      expect(ast[0].else).toBeDefined();
      
      // Verify nested each block in content
    ...
    it('should handle nested define blocks', () => {
      ...
      const template = `
        {{#define getUsers() as users}}
          {{#each users}}
            {{#define getUserDetails(id) as details}}
      ...
      const nestedDefine = eachBlock.content.find(node => node.type === 'define');
      expect(nestedDefine).toBeDefined();
      expect(nestedDefine.expression).toBe('getUserDetails(id)');
      expect(nestedDefine.alias).toBe('details');
    });

    it('should work with single bracket syntax', () => {
      const compiler = new TemplateCompiler();
      const template = `
        {#define getUsers() as users}
          <div>{users.length} users</div>
