...
// string naming an instant reads in the zone asked for. any other string is a wall clock in the
// machine's zone, the way Date reads it, and keeps the day and the clock the person wrote
export const looseZoned = (text, zone) => {
  ...
  return instant.toZonedDateTimeISO(namesInstant.test(text.trim()) ? zoneId(zone) : Temporal.Now.timeZoneId());
};
