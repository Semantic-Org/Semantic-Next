...
`@semantic-ui/dates/schema` registers the seven kinds with `@semantic-ui/schema` as it loads, one `Type`
per class built with `defineType`, and upgrades the built-in `datetime` to `DateTime` under the same
name, so `type: Date` in a schema reads the wrapper. Importing the subpath is the whole opt-in. It
exports the same seven classes as the bare entry and their Types. The bare entry has none of it, and a
bundle that never imports the subpath never pays for it.

```js
import { CalendarDate, CalendarDateType, DateTimeType, DurationType, DateRangeType } from '@semantic-ui/dates/schema';

CalendarDateType.name;                            // 'date'
CalendarDateType.decode('2026-09-06');            // the wire form read back, strictly
CalendarDateType.read('next tuesday');            // dates refused [unreadableDate] next tuesday
CalendarDateType.parse('next tuesday');           // 'next tuesday', left for a schema's validate to flag
CalendarDateType.matchKey(date('2026-09-06'));    // 20260906, equal exactly when equals() holds
CalendarDateType.ordered;                         // true, a range and a sort make sense on a day
DateTimeType.matchKey(now());                     // epoch nanoseconds, a BigInt
DateTimeType.matchKey(new Date());                // the same line, so a Date beside a DateTime compares
DateTimeType.upgrades;                            // Date, the built-in the kind stands in for
DurationType.matchKey(duration('PT90M'));         // 5400000, its milliseconds
DurationType.matchKey(months(2));                 // dates refused [calendarDuration] P2M
DateRangeType.ordered;                            // false, a span has no single order
```
...
months or years has no key, since their length depends on a calendar that does not travel. `DurationType`
alone declares `summable`, since lengths add, and an `encode` that refuses a month the way the key does.
On `DateTimeType` only, `condition(operator, day)` says what a field of instants means for a calendar day,
...
`$gt` the next day's), refusing `noZone` when none is configured, and `upgrades` names the built-in
`Date` it stands in for.
