...
export class Signal {
  ...
  protect(value) {
    ...
    if (this.safety === 'freeze') {
      // already-frozen short-circuits without allocating, so reads stay cheap
      return Object.isFrozen(value) ? value : deepFreeze(value);
    }
    return value;
  }
  ...
  *******************************/
  ...
  // under freeze the stored value is immutable, so every helper produces a
  // fresh value; under reference/none the helpers mutate in place to skip
  // the copy. mutate() detects in-place changes via a pre-mutation clone.
  mutate(mutationFn) {
    if (this.safety === 'freeze') {
      // frozen value can't be mutated in place — fn must return the new value
  ...
  map(mapFunction) {
    if (this.safety === 'freeze') {
      this.currentValue = this.protect(this.currentValue.map(mapFunction));
    }
    else {
  ...
  filter(filterFunction) {
    if (this.safety === 'freeze') {
      this.currentValue = this.protect(this.currentValue.filter(filterFunction));
    }
    else {
      const arr = this.currentValue;
      let writeIndex = 0;
