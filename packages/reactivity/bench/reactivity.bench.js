...
test('Reaction.run — cleanup 5 dependencies', async ({ bench }) => {
...
test('Dependency.changed — notify 10 subscribers', async ({ bench }) => {
  await bench.compare(
    bench('baseline', () => {
...
test('Scheduler.flush — 20 pending reactions', async ({ bench }) => {
  await bench.compare(
    bench('baseline', () => {
      for (let i = 0; i < 20; i++) {
        const r = new ReactionBaseline(() => {});
...
test('Dependency.depend — in active reaction', async ({ bench }) => {
  await bench.compare(
    bench('baseline', () => {
      const r = new ReactionBaseline(() => {});
