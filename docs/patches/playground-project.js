...
import { endWithSlash, forceSkypackRawMode, getRandomString } from './shared/util.js';
...
// Shared TypeScript worker across all playground-project instances.
// Creating one worker per instance is wasteful — the TypeScript compiler is
// large and compileProject() is stateless, so all instances can share it.
let _sharedTypeScriptWorkerApi = null;
function getSharedTypeScriptWorkerApi() {
  if (!_sharedTypeScriptWorkerApi) {
    const url = forceSkypackRawMode(new URL('./playground-typescript-worker.js', import.meta.url));
    let worker;
    if (url.origin === window.location.origin) {
      worker = new Worker(url, { type: 'module' });
    }
    else {
      // Different-origin: fetch once and create a blob URL.
      _sharedTypeScriptWorkerApi = fetch(url.href)
        .then(r => r.text())
        .then(text => {
      ...
      return _sharedTypeScriptWorkerApi;
    }
    _sharedTypeScriptWorkerApi = Promise.resolve(wrap(worker));
  }
  return _sharedTypeScriptWorkerApi;
}
...
let PlaygroundProject = class PlaygroundProject extends LitElement {
  ...
  async _loadProjectFromSource() {
    ...
    switch (source.type) {
      ...
      case 'url':
        {
          const { files, importMap, cdnBaseUrl } = await fetchProjectConfig(new URL(source.url, document.baseURI).href);
          // Note the source could have changed while fetching, hence the
          // double-check here.
          if (source !== this._source) {
            return;
  ...
  async firstUpdated() {
    this._deferredTypeScriptWorkerApi.resolve(await getSharedTypeScriptWorkerApi());
  }
  ...
  async getCompletions(changeData) {
    ...
    if (skipFuzzySearch) {
      completions = completionEntriesAsEditorCompletions(
        (_a = this._completionInfo) === null || _a === void 0 ? void 0 : _a.entries,
        changeData.tokenUnderCursor,
      );
    }
    else {
      completions = sortCompletionItems(
        (_b = this._completionInfo) === null || _b === void 0 ? void 0 : _b.entries,
        tokenUnderCursorAsString,
      );
    }
  ...
  async _getCompletionDetails(filename, cursorIndex, completionWord) {
    const workerApi = await this._deferredTypeScriptWorkerApi.promise;
    const completionItemDetails = await workerApi.getCompletionItemDetails(filename, cursorIndex, {
      importMap: this._importMap,
      cdnBaseUrl: this._getEffectiveCdnBaseUrl(),
    }, completionWord);
...
const fetchProjectConfig = async (url, alreadyFetchedFilenames = new Set(), alreadyFetchedConfigUrls = new Set()) => {
  ...
  const result = await expandProjectConfig(config, url, alreadyFetchedFilenames, alreadyFetchedConfigUrls);
  return {
    ...result,
...
const expandProjectConfig = async (
  config,
  baseUrl,
  alreadyFetchedFilenames = new Set(),
  alreadyFetchedConfigUrls = new Set(),
) => {
  var _a, _b, _c, _d, _e;
  const filePromises = [];
    ...
    if (info.content === undefined) {
      filePromises.push((async () => {
        ...
        return {
          ...
          contentType:
            (_b = (_a = resp.headers.get('Content-Type')) === null || _a === void 0 ? void 0 : _a.toLowerCase())
                !== null && _b !== void 0
              ? _b
              : 'text/plain',
        };
  ...
  if (extendsConfigPromise) {
    ...
    cdnBaseUrl = cdnBaseUrl !== null && cdnBaseUrl !== void 0 ? cdnBaseUrl : extendsConfig.cdnBaseUrl;
  }
...
const outdent = (str) => {
  ...
  return str.replace(
    RegExp(`^\\s{${shortestIndent !== null && shortestIndent !== void 0 ? shortestIndent : 0}}`, 'gm'),
    '',
  );
};
/**
