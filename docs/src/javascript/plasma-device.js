...
const DEFAULTS = {
  ...
  /** Starting time offset. 'random' = Math.random() * 1000, or a number for a fixed pattern. */
  seed: 'random',
...
function createWebGLRenderer(canvas, getCfg) {
  ...
  const self = {
    ...
    resize() {
      ...
      // Render at CSS pixel resolution — on Retina displays the browser's
      // 2x downsampling acts as free anti-aliasing for procedural textures.
      // This is intentional: DPR sharpness helps text/UI but creates moiré
      // on high-frequency patterns like plasma bands.
...
function createCanvasRenderer(canvas, getCfg) {
  ...
  return {
    ...
    resize() {
      ...
      w = Math.ceil(rect.width / divisor);
      h = Math.ceil(rect.height / divisor);
      canvas.width = w;
      canvas.height = h;
...
function createFpsSystem(canvas, getCfg, getRendererInfo) {
  ...
  function tick(now) {
    ...
    if (infoEl) {
      ...
      infoEl.textContent = `${info.w}×${info.h} · ${info.type} · ${gamut} · m(${mx}, ${my})`;
    }
...
export function createPlasma(selector, settings = {}) {
  ...
  let time = cfg.seed === 'random' ? Math.random() * 1000 : cfg.seed;
  let animId = null;
  let running = false;
  let destroyed = false;
  ...
  return {
    ...
    /** The resolved seed (capture this when seed='random' to reproduce a pattern) */
    seed: time,
