...
```sui
{formatDate time "h:mm:ss a" dateSettings}
```

The new value `dateSettings` is retreived from the component instance and returns a settings object for use with `formatDate`.
