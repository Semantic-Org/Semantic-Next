...
This is especially useful when working with more complex templates or when you need to use the index in calculations:

```sui
{#each item, position in items}
  <div class="item {position % 2 === 0 ? 'even' : 'odd'}">
