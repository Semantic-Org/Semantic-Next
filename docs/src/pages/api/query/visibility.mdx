...
> **Note** This method finds the actual containing block that coordinates like `top` and `left` are relative to when positioning elements. The native `offsetParent` property predates modern CSS and doesn't account for stacking contexts created by `transform`, `will-change`, `filter`, `perspective`, and `contain`.
...
// Position element relative to containing parent
