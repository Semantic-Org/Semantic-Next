...
> **Note**: Be cautious when using early returns with `firstRun`. If you return early on the first run, the reaction won't establish dependencies on signals accessed after the return statement.
