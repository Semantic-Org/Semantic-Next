...
Dependent signals are [signals](/reactivity/signals) whose values automatically update when other signals change. Semantic UI provides two methods for creating dependent signals:

- **[`signal.derive()`](/api/reactivity/signal#derive)** - Transform a single signal into a new signal
- **[`Signal.computed()`](/api/reactivity/signal#computed)** - Combine multiple signals into a computed value

## Derived Signals

<PlaygroundExample id="reactive-derived" />

Use [`derive()`](/api/reactivity/signal#derive) to create a new signal that transforms the value of a single source signal. The derived signal automatically updates whenever the source signal changes.
...
Use [`Signal.computed()`](/api/reactivity/signal#computed) to create a signal that depends on multiple other signals. The computed signal recalculates whenever any of its dependencies change.
...
Both derived and computed signals work seamlessly with [reactions](/reactivity/reactions), allowing you to create reactive side effects based on dependent values.
