...
The default options are designed to permit some minor performance overhead as a tradeoff for improved developer experience, but this might not fit all use-cases for [Signals](/reactivity/signals).
...
You can provide a custom equality comparison function using the [`equalityFunction`](/api/reactivity/signal#options) option to modify how equality checks are performed. This can potentially speed up comparisons at the cost of potential additional reactivity.
...
By default, Signals automatically clone object and array values during [`get`](/api/reactivity/signal#get) and [`set`](/api/reactivity/signal#set) operations.
...
Disabling cloning (using the [`allowClone`](/api/reactivity/signal#options) option) will reduce the overhead of `set` and `get` calls but will potentially cause unexpected behaviors when modifying arrays and objects.
...
Similar to the equality check, the cloning function itself can be customized by providing a [`cloneFunction`](/api/reactivity/signal#options) in the constructor options.
