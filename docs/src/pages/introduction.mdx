...
<UICards four horizontal>
  ...
  <UICard horizontal
    ...
    icon="cpu"
    href="/components"
    description="A signals based reactivity library that is integrated into components but can also be used by itself."
  ></UICard>
  <UICard horizontal
    header="@semantic-ui/query"
    icon="mouse-pointer"
    href="/query"
    description="A modern 3kb jQuery-like replacement to handle DOM APIs."
  ></UICard>
  <UICard horizontal
    header="@semantic-ui/utils"
    icon="tool"
    href="/api/utils"
    description="Create custom UI components that render as web components."
  ></UICard>
</UICards>
