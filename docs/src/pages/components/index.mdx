...
Semantic UI components are standard Web Components that are enhanced with built-in features designed to simplify common UI development tasks like managing state, rendering templates, handling events, and styling.
...
* [Templating](/components/rendering) - A reactive templating system to render web components with [reactive expressions](/templates/expressions), [snippets](/templates/snippets), [subtemplating](/templates/subtemplates), [template helpers](templates/helpers), [loops](/templates/loops), [conditionals](/templates/conditionals), and more. An integrated [signals based reactivity system](/components/reactivity) make for precision updates. Supports [full composable components](/templates/subtemplates#basic-example) in a similar fashion to React, Vue, Svelte or other front end frameworks.
* [CSS Design System](/components/styling) - Robust design system based off CSS variables, CSS layers, and HDR colorspaces.
* [DOM Manipulation](/query) - Use a chainable DOM querying library that is built to harness the encapsulation of the shadow DOM.
* [Events](/components/events) - Define events that occur inside your component, attach to the page, attach to deeply nested components and more. Dispatching events to passing data through the DOM to other components.
* [Keybindings](/components/keys) - Add keybindings directly to your components using a simple declarative format that supports key sequences, key combinations and more.
...
> **Using Frameworks like React, Vue, Svelte**  All major frameworks support web components meaning you can use them directly in any framework. Web components can also render in the browser natively without compilation.
