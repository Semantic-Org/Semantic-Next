...
The `createInstance` callback can be used when [creating a component](/components/create) to define the behaviors of your component so that they can be accessed in various other locations that might need to reference or invoke them.
...
Component instances are passed through to all other callbacks as `self` allowing you to reference methods and values directly from [events](/component/events), [key bindings](/components/keys), and [lifecycle callbacks](/components/lifecycle).
...
```javascript
const createInstance = ({ findChildren }) => {
  getRowName(id) => {
...
```javascript
const createInstance = ({ findTemplate }) => {
  getTodos() => {
