...
The `createComponent` callback defines the behaviors of your component when [creating a component](/components/create). These behaviors can be accessed from various locations that might need to reference or invoke them.
...
You can access other data like [`settings`](/components/settings), [`state`](/components/state), as well as the DOM using [`$` and `$$`](/query) from inside your component instance, allowing you to manipulate data from the DOM, access sources of [reactivity](/components/reactivity), and settings passed through to your component.
...
You can use Query's [`component()`](/api/query/components#component) helper to easily access a component instance
