...
Your components [`state`](/components/rendering#state), [`settings`](/components/rendering#settings), template instance `tpl` and other data sources are passed through to each [lifecycle arguments](/components/lifecycle#callback-arguments) like `createInstance`.

> For more information on `createInstance` see our dedicated subsection on [defining functionality](/components/instances).
...
createComponent({
  keys: {
    'up': ({ tpl }) => tpl.selectPreviousResult(),
    'down': ({ tpl }) => pl.selectNextResult(),
    'esc': ({ tpl }) => tpl.blurSearch(),
  }
});
```
...
If you are using build tools to compile your project, you can use the `?raw` query parameter to import a file directly as a text string.
