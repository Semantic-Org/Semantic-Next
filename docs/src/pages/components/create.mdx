...
Your components [`state`](/components/rendering#state), [`settings`](/components/rendering#settings), component instance `self` and other data sources are passed through to each [lifecycle arguments](/components/lifecycle#callback-arguments) like `createInstance`.
