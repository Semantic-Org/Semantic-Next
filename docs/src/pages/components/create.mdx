...
The minimum requirement to render a template is an html template.
...
In most application you will need to do many more things provided by a larger framework like Svelte, React or Vue. `createComponent` provides features that allow this to be accomplished with web components.

* **Functionality** - An object returned from `createInstance` defines what methods and properties are attached to the DOM element associated with the component which provide behaviors. For instance a popup might have a `show` and `hide` behavior.
...
> **Destructured Params** - Semantic callbacks provide a variety of parameters that are returned from functions called inside the component and allow you to access data and
