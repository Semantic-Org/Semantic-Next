...
Semantic provides a standalone library called [Query](/query) designed to work with the Shadow DOM for accessing the DOM. It's built with modern ECMAScript and browser standards, and tightly coupled into the component framework.
...
> This section covers only the specific integration of Query inside of components, for a full explanation of the library check out the [Query documentation](/query).
