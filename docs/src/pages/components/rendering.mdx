...
* [Conditionals](/api/templates/conditionals) - Branching structures like `if` `else if` and `else`
* [Looping structures](/api/templates/loops) - Structures to iterate through data like arrays or objects
* [Reactive data](/api/templates/expressions) - State stored with your component that should update the page when it updates
* [Function calls](/api/templates/expressions) - Methods on your component that allow you to access content and perform tasks
* [Subtemplates](/api/templates/subtemplates) - Reusable pieces of html organized as a separate file
* [Snippets](/api/templates/subtemplates) - Reusable pieces of html included elsewhere in a template
* [Slots](/api/templates/slots) - Locations where content can be placed inside your component
* [Snippets](/api/templates/snippets) - reusable html structures inside a component


> For more information on all the features of templates check our dedicated subsection [on templates](/api/templates).
...
* **[Settings](#settings)** - A value `name` passed in via html attribute or property
* **[State](#state)** - An internal reactive state called `name`
* **[Subtemplate Data](/api/templates/subtemplates)** - Data passed in from a parent template called `name`
...
[Global helpers](/api/templates/helpers) are special utilities that are available across all templates and provide utilities like `classMap`, `formatDate`, `concat` `stringify` that can be used to format values for output.

> For a list of global helpers please visit the dedicated [global helper page](/api/templates/helpers) in templating.
...
For a complete list of available global helpers see [global helpers](/api/templates/helpers).
...
#### Inside a Component
You can use state inside a component by destructuring `state` from any [lifecycle events](/components/lifecycle#callback-arguments).
...
> For more information see [lifecycle events](/components/lifecycle#callback-arguments).
...
When no tag name is specified they are returned as [Subtemplates](/api/templates/sub-templates) which can be passed in to other components.
