...
Templates provide primitives for constructing html like [control structures](/templates/control), [expressions](templates/data),  content slots, conditionals, subtemplates, snippets and more.
...
This will check each of these locations in sequence and return the first available value.
...
Expressions are evaluated right to left with values being retreived from the data context.
```html
