...
### Definining Functionality
Your template instance stores methods and properties that implement the functionality of your component. You can access your template instance directly from [lifecycle events](/lifecycle/events) as `tpl` to invoke methods or access properties.
...
You can access your template instancec as `tpl` from other callbacks to invoke functionality.

```javascript
const onRendered = ({tpl}) => {
  tpl.incrementCounter();
};
```
