...
Keywords are special leading terms that modify how events are attached permitting you to bind events to elements other than those found in your [component's template](/templates).
...
```javascript
const events = {
  'click .increment'({tpl, data]}) {
    tpl.increment(data.amount);
  }
};
```
...
const onCreated = ({attachEvent, settings, isClient}) => {
  if(isClient) {
    attachEvent(settings.scrollContext, 'scroll', tpl.onPageScroll)
  }
};
```
...
```javascript
const events = {
  'rendered inpage-menu'({tpl,data}) {
    // menu component is rendered lets call something on it
    const menu = data.tpl;
    menu.setActive();
  }
