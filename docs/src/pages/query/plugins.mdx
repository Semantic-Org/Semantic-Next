...
- **[Simple Plugins](#simple-plugins)** - Add methods directly to [Query instances](/query/basics)
- **[Complex Plugins](#complex-plugins)** - Create full behavioral systems with settings and events
...
$.plugin.highlight = function(options = {}) {
  return this.each((element) => {
    // Plugin logic per element
    element.style.backgroundColor = options.color || 'yellow';
  });
};

// Now available on all Query instances
$('.searchResults').highlight({ color: 'lightblue' });
```

The `$.plugin` object is an alias for `Query.prototype`. Methods added here become available on all Query instances with full access to [chaining](/query/chaining) and the Query API.

<PlaygroundExample id="query-plugin" />

Use simple plugins for utility methods that don't require state management or complex configuration.
...
**Simple plugins** are for adding one utility method to Query (like jQuery plugins). Use them when you want to extend Query with DOM manipulation helpers that don't need configuration or event handling.

**Complex plugins** are for creating full behaviors with settings and events (like classic Semantic UI modules). Use them when building interactive components that need state management, configuration options, or complex event handling.

> **Registration** Plugins can self-register through side-effect imports, enabling modular architecture where `import '@semantic-ui/query/plugins/transition'` makes the plugin available across your application.
...
This example demonstrates the three-tier settings precedence system: plugin defaults are overridden by settings objects, which are overridden by data attributes. This hierarchy gives users flexible control over plugin behavior.
