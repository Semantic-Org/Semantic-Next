...
> **API Reference:** For comprehensive documentation of all functions, visit the [API reference guide](/api/query).
...
- **[$](/query/basics)** - Efficiently queries the standard DOM tree using native DOM APIs
- **[$$](/query/shadow-dom)** - Seamlessly queries across shadow DOM boundaries
...
Query embraces [method chaining](/query/chaining), allowing you to perform multiple operations in a fluent, readable sequence:
...
Query is deeply integrated into the Semantic UI component system. Both `$` and `$$` are automatically available in all [component lifecycle methods](/components/lifecycle) and event handlers:
...
> Explore the [Components integration guide](/query/components) to learn more about using Query within Semantic UI components.