...
The Query constructor creates a new Query collection from a variety of input sources.
...
// Find elements with slot attribute across shadow boundaries
const $slottedElements = $$('my-component [slot="header"]');

// Get content from slots
const headerContent = $('my-component').getSlot('header');
```
...
> **SUI Components** Semantic UI components use this technique when returning `$` and `$$` [inside components](/docs/guides/components/dom#using-query-in-components)
...
Restore the previous values of `$` and `$$` and return the Query function.
...
// If $ was previously defined, restore it and use a different alias
const myQuery = $.noConflict();
const $divs = myQuery('div');
```

### Example
<PlaygroundExample id="query-noconflict" direction="horizontal"></PlaygroundExample>

## ESM Usage

Examples of using Query with ES modules.
...
Examples of using Query in browser environments.
