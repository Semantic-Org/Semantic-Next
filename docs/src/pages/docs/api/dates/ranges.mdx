...
The ends and the kind are real properties, so a range prints them in a console without a click, and `text` leads them with both ends spelled the way a `Date` prints, so a preview reads as a span. Every property is read-only.

| Name | Type | Description |
|------|------|-------------|
| text | string | Both ends' `text` joined with `to`, `'Tue Sep 01 2026 to Mon Sep 07 2026'` |
...
week.text; // 'Tue Sep 01 2026 to Mon Sep 07 2026'
