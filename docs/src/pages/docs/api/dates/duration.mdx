...
The fields are real properties, as written, so a value prints them in a console without a click, and `text` leads them spelled out, so a preview reads as a length. Every property is read-only.

| Name | Type | Description |
|------|------|-------------|
| text | string | The fields as written, spelled out, `'1 hour, 30 minutes, 15 seconds'` |
| years | number | |
| months | number | |
| weeks | number | |
| days | number | |
