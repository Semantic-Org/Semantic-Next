...
The parts are real properties, and `text` leads them spelled the way a `Date` prints its clock, so a preview reads as a time. Every property is read-only.

| Name | Type | Description |
|------|------|-------------|
| text | string | The clock the way a `Date` prints it, `'17:30:15'` |
