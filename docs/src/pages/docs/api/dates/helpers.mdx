...
```javascript
weekdayNames();
weekdayNames(locale, style, firstDay);
```
...
weekdayNames(undefined, 'short', 'sunday'); // ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']
weekdayNames('de-DE', 'long', 'sunday')[0]; // 'Sonntag'
```
...
monthNames(undefined, 'short')[8]; // 'Sep'
