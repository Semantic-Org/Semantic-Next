...
Scopes a stylesheet under a selector, with configurable options for web component integration and root element handling. It reads the sheet through `parseCSS`, so it runs on the server as well as in the browser, and nested rules stay under their scoped parent. Every selector in a top-level rule and in the rules inside `@media`, `@supports`, `@layer` and `@container` blocks gains the scope, while keyframe selectors are left alone. The output is written in the canonical layout of `stringifyCSS`.
...
| at-rule | `{ type: 'at', name, prelude, children: [...] }` for a block (`@media`, `@layer name`, `@font-face`), `{ type: 'at', name, prelude }` for a statement (`@import`, `@layer a, b;`) |
...
// [{ type: 'at', name: 'media', prelude: '(min-width: 40em)', children: [
//   { type: 'rule', selectors: ['.a'], children: [...] },
// ] }]

parseCSS('@import url("x.css") layer(base);');
// [{ type: 'at', name: 'import', prelude: 'url("x.css") layer(base)' }]
