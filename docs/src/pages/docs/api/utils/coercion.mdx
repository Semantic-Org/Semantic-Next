...
```javascript
function toBoolean(value, { falsy, loose = false } = {})
```
...
|-------|----------------------|---------|-------------|
| falsy | string \| string[]   | —       | Extra tokens to treat as false, layered over the built-in set |
...
Coerces a value to an integer by truncating toward zero, or `null` when it's unconvertible or non-finite. Equivalent to `toNumber` followed by a truncation, so `Infinity` joins the `null` bucket.
...
```javascript
function toString(value, { loose = false } = {})
```
