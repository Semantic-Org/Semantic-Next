...
methods: [get, has, set, unset, elementKey, pathKey, isPathKey, splitPath, parsePath, eachPath, pathFrom, elementPath, keyedPath, patternFrom, expandPath, pathCovers, pathsOverlap]
icon: route
...
The identity of an array element — the value of the first present field in `keys`, or `undefined` for a scalar or an object carrying none of them. The same convention as reactivity's `Signal.id` and the renderer's `getItemId`, and what the keyed [`detectChanges`](/docs/api/utils/objects#detectchanges) mode and the keyed [`get`](#get) / [`set`](#set) / [`unset`](#unset) grammar match on.
...
```javascript
function pathKey(item, keys = ['id', '_id', 'hash', 'key'])
```
...
```javascript
function patternFrom(path)
```

Collapse a concrete path to its pattern spelling — `lines[#a1].tax` becomes `lines.*.tax`.

#### Parameters

| Name | Type   | Description |
|------|--------|-------------|
| path | string | The concrete path to collapse |

#### Returns

The pattern spelling, or the input path unchanged when it doesn't parse.

#### Notes

Every element address collapses, keyed and positional alike, so per-element paths group under one dependency pattern instead of one entry per element. The resulting pattern always [covers](#pathcovers) the path it came from.
...
console.log(patternFrom('lines[#jack@semantic-ui.com].tax')); // 'lines.*.tax'
console.log(patternFrom('lines.2.tax'));                      // 'lines.*.tax'
console.log(patternFrom('invoice.total'));                    // 'invoice.total'

// per-element writes group under one pattern
const written = ['lines[#a1].tax', 'lines[#a2].tax', 'lines[#a1].qty'];
console.log([...new Set(written.map(patternFrom))]); // ['lines.*.tax', 'lines.*.qty']

// the pattern still covers the concrete path it came from
console.log(pathCovers(patternFrom('lines[#a1].tax'), 'lines[#a1].tax')); // true
```

### expandPath

```javascript
function expandPath(path, { from, doc, keys = ['id', '_id', 'hash', 'key'] } = {})
```
...
| keys | string[] | `['id', '_id', 'hash', 'key']` | Identity fields used while enumerating |
