...
Only a segment whose element carries a path-safe identity (any id without `]`, see [`elementKey`](#elementkey)) rewrites. Keyless arrays, inner value lists, unresolvable paths, and already-keyed spellings pass through unchanged. A leading index (the root itself being an array) stays positional, since [`get`](#get) and [`set`](#set) can't parse a leading bracket. The emitted `[#id]` paths apply back through [`get`](#get)/[`set`](#set)/[`unset`](#unset) and match the keyed output of [`detectChanges`](#detectchanges).
