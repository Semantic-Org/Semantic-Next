...
```javascript
function trackWrites(value, callback, { strategy = 'auto', returnPaths = true, keyed = true, fields = elementKey.config.fields, onWrite, clone, equality } = {})
```
...
| fields      | string[] | `elementKey.config.fields` | Identity fields for keyed paths, first present wins |
...
```javascript
function trackReads(value, callback, { returnPaths = true, keyed = true, fields = elementKey.config.fields, onRead } = {})
```
...
| fields  | string[] | `elementKey.config.fields` | Identity fields a keyed element is matched on |
...
| Name               | Type    | Default | Description |
|--------------------|---------|---------|-------------|
| preserveNonCloneable | boolean | true    | Keep a class instance as is instead of flattening it to a plain object |
| preserveDOM        | boolean | true    | Keep a DOM node as is instead of walking it |
