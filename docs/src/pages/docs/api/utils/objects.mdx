...
methods: [keys, values, filterObject, mapObject, trackWrites, extend, assignInPlace, deepExtend, pick, arrayFromObject, get, proxyObject, onlyKeys, hasProperty, reverseKeys, weightedObjectSearch]
icon: box
...
```javascript
function trackWrites(value, callback, { strategy = 'auto', onWrite, clone, equality } = {})
```

Runs a callback against a value and reports whether the callback changed it. Useful for running a user callback against a data structure and determining mutations.
...
| equality | function | isEqual   | Equality used to compare snapshots |

#### Returns

`{ changed, result }` — whether the value changed, and the callback's return value.

#### Notes

Under the proxy strategy the tracked wrapper is only valid inside the callback — using one after it returns throws.
...
// report each write with its path
