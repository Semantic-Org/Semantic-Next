...
methods: [log, createLogger, warnOnce, createErrors, error, throwError]
icon: bug
...
| [`warnOnce(key, message, options)`](#warnonce) | Logs at `warn` level the first time this logger sees `key`, silent after |
...
Development advice keeps its guard at the callsite, `isDevelopment && warnOnce(...)`, so a bundler folds the call and its message string out of a production build together (see [Development Messages](#development-messages)). An operational line that must reach production calls it bare.
...
| options | object | Any [`log` option](#options), merged over the logger's defaults exactly as the plain member merges them |
