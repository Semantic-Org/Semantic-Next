...
Every message leads with one uniform line, `<layer> refused [<code>] <at>`, parseable with `/^(\S+) refused \[([\w-]+)\] (.*)$/m` — so a support ticket carrying a pasted error still names the layer, the code, and the address. In development the explanation follows on its own line; in production the line is the whole message.
