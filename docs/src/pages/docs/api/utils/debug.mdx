...
The Debug utilities provide the two reporting doors a layer needs: `log` for styled console output, and `createErrors` for coded errors that carry a teaching message in development and one uniform, greppable line in production.
...
Creates a layer-bound error toolset. Bind it once per layer and every refusal that layer raises reads the same way: a stable `code` you can route on, an address (`at`) that says where it happened, and a development `explanation` that folds out of production builds.

Production keeps one uniform line, `<layer> refused [<code>] <at>`, parseable with `/^(\S+) refused \[([\w-]+)\] (.*)$/` — so a support ticket carrying a pasted error still names the layer, the code, and the address.
...
| at      | string | **Where** it happened: the greppable address, like `todos:secret.field` or `db-1 -> db-2`. Say **why** in the explanation |
| options | object | Optional configuration |
...
| Name        | Type          | Default   | Description |
|-------------|---------------|-----------|-------------|
| explanation | string \| `0` | undefined | The development teaching message. When present it becomes the error's message; when folded away the message is the production line. See [Development Messages](#development-messages) |
| detail      | any           | undefined | Structured data attached to the error as `error.detail` |

#### Returns

The `Error` it reported, carrying `code` and (when given) `detail`. Returning it lets a caller attach it to local state or a result envelope without building a second one.
...
console.log(reported.message); // 'sync refused [writeRejected] todos:a7f2' in production
```
...
The string `<layer> refused [<code>] <at>`, or `refused [<code>] <at>` when the toolset was created without a `layer`.
...
const [, layer, code, at] = 'sync refused [storageChanged] db-1 -> db-2'
  .match(/^(\S+) refused \[([\w-]+)\] (.*)$/);
```

## Development Messages

`explanation` is teaching prose. It belongs in a development build and nowhere else, so fold it **at the callsite**, in argument position:
