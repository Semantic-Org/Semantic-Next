...
> **Promise Semantics** Pending promises resolve with the result of the eventual execution. `cancel()` rejects them with code `CANCELLED`, and `rejectSkipped` rejects superseded calls with code `DEBOUNCED`, so an awaited debounced call needs a catch.
