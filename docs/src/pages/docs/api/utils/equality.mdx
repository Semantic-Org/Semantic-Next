...
> **Deep Comparison** This function performs a recursive comparison for objects and arrays, ensuring that nested structures are also checked for equality. It handles various edge cases, including `NaN` comparisons. A class instance compares by its own `equals()` when both operands carry one, the convention Temporal, Immutable.js and Luxon share, so two `Temporal.Instant` values are equal when they mark the same instant.
...
| ignoreKeys | string[] | `[]` | Array of property names to skip during comparison, at the top level only |
| deepIgnore | boolean | `false` | Applies `ignoreKeys` at every depth instead of just the top level |
...
console.log(isEqual(Temporal.Instant.from('2026-09-06T14:30Z'), Temporal.Instant.from('2026-09-06T14:30Z'))); // true (by its own equals)
```
