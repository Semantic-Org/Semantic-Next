...
> **Deep Comparison** This function performs a recursive comparison for objects and arrays, ensuring that nested structures are also checked for equality. It handles various edge cases, including `NaN` comparisons. A temporal value, a datetime from `@semantic-ui/dates` or a native Temporal value, compares by its own `equals()`, so two datetimes are equal when they are the same instant, whatever zone each reads in.
