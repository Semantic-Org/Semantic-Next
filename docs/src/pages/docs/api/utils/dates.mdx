...
```javascript
function formatDuration(value, { decimals, unit, lossless, mixed, separator } = {})
```
...
`mixed` is for a person reading a wait or a lap time, where `'6.5m'` is not how anyone says six and a half minutes. It prints the clock form, whole units and the remainder in the next: `390000` is `'6:30'`, an hour and five minutes is `'1:05:00'`, and below a minute it is `'49s'`. The value rounds to the whole second (`59600` is `'1:00'`), a positive value never prints as zero (`400` is `'400ms'`), the hours are unbounded past a day, and `decimals`, `lossless` and `unit` do not apply. It is the one print that does not read back through `toDuration`.
...
| mixed    | boolean | `formatDuration.config.mixed` (false) | Print the clock form, `'6:30'`, `'1:05:00'`, `'49s'` below a minute |
...
console.log(formatDuration(390000, { mixed: true })); // '6:30'
console.log(formatDuration(3900000, { mixed: true })); // '1:05:00'
console.log(formatDuration(49400, { mixed: true })); // '49s'
