...
```javascript
function formatDuration(value, { decimals, unit, lossless } = {})
```
...
`decimals` is a maximum, so `'5m'` never prints as `'5.0m'`, and a value that rounds up to a whole unit promotes (`3598200` is `'1h'` at one decimal, not `'60m'`). `unit` holds one unit so a column reads down in the same scale, printed as spelled, so `'minutes'` gives `'1.5minutes'`.
