...
`format` picks the form, the same word `formatDate` takes. `'decimal'`, the default, is everything above. The other two are for a person reading a wait or a lap time, where `'6.5m'` is not how anyone says six and a half minutes. `'clock'` prints whole units and the remainder in the next: `390000` is `'6:30'`, an hour and five minutes is `'1:05:00'`, the hours unbounded past a day. `'units'` prints the two largest non-zero whole units with a space: `'6m 30s'`, `'1h 5m'`, `'1d 5m'`. Both read `'49s'` below a minute, round to the whole second (`59600` is `'1:00'` and `'1m'`), never print zero for a positive value (`400` is `'400ms'`), and keep the sign; `decimals`, `lossless` and `unit` apply to the decimal form alone. Neither reads back through `toDuration`, which reads one quantity, and a word outside the three answers `null` like an unknown unit.
...
| format   | string  | `formatDuration.config.format` (`'decimal'`) | `'decimal'` one unit with decimals, `'clock'` for `'6:30'` and `'1:05:00'`, `'units'` for `'6m 30s'` and `'1h 5m'` |
...
console.log(formatDuration(390000, { format: 'units' })); // '6m 30s'
console.log(formatDuration(3900000, { format: 'units' })); // '1h 5m'
console.log(formatDuration(49400, { format: 'units' })); // '49s'
