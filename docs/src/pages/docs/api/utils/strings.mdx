...
methods: [kebabToCamel, camelToKebab, capitalize, capitalizeWords, toTitleCase, joinWords, getArticle, truncate, tokenize, escapeHTML, unescapeHTML, reverseString]
icon: type
...
```javascript
function humanize(str = '', { titleCase = false, dropId = true } = {})
```

Converts a machine identifier into human-readable label text, the display-side inverse of `tokenize`. Splits `snake_case`, `kebab-case`, `camelCase`, and `PascalCase`, keeping acronym runs intact, and sentence-cases the result.
