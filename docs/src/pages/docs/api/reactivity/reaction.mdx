...
methods: [create, nonreactive, guard, run, stop, invalidate, firstRun, active, context, dependencies, flush, scheduleFlush, afterFlush, getSource]
...
`Reaction` proxies several useful methods from [`Scheduler`](/docs/api/reactivity/scheduler) so you can call them directly without having to import the [`Scheduler`](/docs/api/reactivity/scheduler) class.
