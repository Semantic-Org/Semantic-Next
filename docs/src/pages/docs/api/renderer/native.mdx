...
There is no render loop, no virtual DOM, and no component-level re-render. The AST is walked once, when the instance first renders. After that, updates flow only through the reactions bound during that walk.

Rendering is a one-time wiring step. Reactivity is what runs afterward.
