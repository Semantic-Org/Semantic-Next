...
```javascript
import { renderToStaticMarkup } from '@semantic-ui/component';

const html = renderToStaticMarkup(Card, { title: 'Hello' });
// <div class="card">Hello</div>
```

Slots fill in place from the `slots` option, there being no light DOM to project. A nested component renders where its tag sat, with its children assigned to its slots by their `slot` attribute.
