...
Represents a value-based branch on a single discriminant. Each branch is an `is` case or an `else` fallback, evaluated in order with the first match winning.
...
```javascript
{
  ...
  branches: [
    {
      type: 'is',
      values: ["'loading'", "'pending'"],  // case values, matched loosely (==)
      content: [/* nested AST nodes */]
    },
    {
      type: 'is',
