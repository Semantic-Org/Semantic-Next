...
Manifests a new Template instance from a prototype template. The renderer calls `clone` to produce per-mount instances from a registered prototype — each mount gets its own state, instance, and DOM bindings while sharing the prototype's compiled AST, CSS, and callbacks.

This is not a deep copy. The returned Template shares structural definitions (AST, events, CSS, `createComponent`) with the source by reference; only per-instance fields (data, element, parent linkage) are fresh.
...
| Name     | Type   | Description                                                    |
|----------|--------|----------------------------------------------------------------|
| settings | Object | Per-instance overrides — typically `data`, `element`, `parentTemplate` |
