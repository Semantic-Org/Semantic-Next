...
A subtemplate normally sees only what its call passes. Adding `inheritsData` gives it the caller's data context the way a snippet has it, settings and state live per key and other values as they were when it rendered, with the call's own props on top. Inherting data is opt-in because in single page apps deeply nested subtemplates would cause cascading inheritance which increases memory usage and reduces performance.
