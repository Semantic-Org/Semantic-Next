...
Inside JS callbacks, `settings` falls back to the host component's settings for keys the subtemplate didn't declare. In template expressions only the subtemplate's own declared settings are reactive; pass host settings explicitly to use them in markup.
