...
You can use `this` to refer to the entire element in the loop
```sui
{#each number}
  {this}
{/each}
...
You can also use any JS to evaluate [expressions](/docs/guides/templates/expressions) in your data context
```sui
{#each [1, 2, 3]}
  {this}
{/each}
...
These [expressions](/docs/guides/templates/expressions) can also use variables
```sui
{#each [getFirst, getSecond, 3]}
  {this}
{/each}
```
...
This can be used with [helpers](/docs/guides/templates/helpers) like `numberFromIndex` to output non-zero indexed values.
...
[Global helpers](/docs/guides/templates/helpers) can be used to generate iterables like number ranges.
...
The following example uses loops with [snippets](/docs/api/templates/snippets) to output values into a table.
