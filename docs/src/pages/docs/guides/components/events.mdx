...
No-selector handlers bind on the host element, so events on the host's own surface — padding, border, host-dispatched custom events — fire alongside events from inside the component.
...
> See [Projection vs. Piercing](#deep-events) for how this interacts with slotted content and shadow boundaries.
...
When no selector is provided, the event attaches to `window` — handy for `scroll`, `hashchange`, and similar page-level events.
...
Pass a selector to target a specific element instead:
...
const events = {
  'deep ui-button .icon'() {
    // the icon lives inside ui-button's shadow tree —
    // requires deep to pierce that boundary
  }
};
```

> **Composed events required** `deep` only catches events that bubble out of a child component's shadow tree, which means the inner element must dispatch with `composed: true`. Native events (`click`, `mouseover`, etc.) already do this. For custom events, use the framework's [`dispatchEvent`](#dispatching-custom-events) helper — it sets `composed: true` by default.
