...
> **Composed events required** `deep` works for events that bubble out of a child component's shadow tree, which means the inner element must dispatch with `composed: true` (native events like `click` already do this; CustomEvents need to opt in).
