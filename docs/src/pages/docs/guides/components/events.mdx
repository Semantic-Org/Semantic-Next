...
| data              | data attributes, template scope at the target, and event.detail — merged in that order, later sources win |
...
`data` merges three sources: data attributes on the dispatching element, the template scope at that element, and `event.detail`. When keys overlap, later sources win.
...
<PlaygroundExample id="event-scope" direction="horizontal"></PlaygroundExample>
