...
Default settings are specified by passing a `defaultSettings` object when [creating a component](/docs/guides/components/create) using [`defineComponent`](/docs/api/component/define-component):
...
Settings represent the external API for your component, while [state](/docs/guides/components/state) represents the internal configuration of your component.

For example, in a dropdown component:
- The list of options would be a **setting** (configured from outside)
- Whether the dropdown is currently open would be **[state](/docs/guides/components/state)** (managed internally)
...
The [Query library](/docs/guides/query) provides methods for accessing components and updating their settings programmatically.

#### The settings() Method

Use the [`settings()`](/docs/api/query/components#settings) method to set multiple properties on one or more components:
...
Use the [`setting()`](/docs/api/query/components#setting) method to set a single property on one or more components:
...
The [`initialize()`](/docs/api/query/components#initialize) method sets properties after the DOM is fully loaded:
