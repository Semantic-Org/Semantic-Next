...
Semantic provides a standalone library called [Query](/docs/guides/query) designed to work with the Shadow DOM for accessing the DOM. It's built with modern ECMAScript and browser standards, and tightly coupled into the component framework.

### Use of Query

[Query](/docs/guides/query) provides a chaining interface similar to jQuery for accessing the DOM.
...
> This section covers only the specific integration of Query inside of components, for a full explanation of the library check out the [Query documentation](/docs/guides/query).
...
The `$` and `$$` arguments are available from [lifecycle callbacks](/docs/guides/components/lifecycle#callback-arguments) and allow you to manipulate the DOM from your component.
...
The following example uses page-level Query (imported directly) to highlight what each operator can reach. Inside a component the same rules apply, scoped to the component's shadow root.
...
Query provides a helper `component` which can be used to access your components instance from anywhere in the DOM.

This can be used to trigger functionality defined on your component from anywhere in your page.
...
You can use `settings` to initialize a component **after it is included in the page** with settings programmatically. This can be particularly useful for components that can be initialized with functions.
