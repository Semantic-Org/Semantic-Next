...
Your component's `state` is the internal store of reactivity for your component. When your component is initialized each `state` property will become a [`signal`](/docs/guides/reactivity/variables) and will trigger reactivity inside a `Reaction`.
