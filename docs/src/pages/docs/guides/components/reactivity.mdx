...
This guide focuses primarily on reactivity in the context of components, if you want to learn about how reactivity works generally check out the [reactivity package guide](/docs/guides/reactivity).
...
> **Avoiding Reactivity** You can access reactive values with `peek` to avoid reactivity, and use `Reaction.nonreactive` to control the reactivity of a function. For more information see [controlling reactivity](/docs/guides/reactivity/controls).
...
Each `state` property becomes a [`signal`](/docs/guides/reactivity/signals) when the component is initialized. Accessing state inside a `reaction` creates a dependency that reruns when the value changes.
...
const onCreated = ({state, reaction}) => {
  ...
  state.counter.increment(); // reaction reruns
}
```

State is also reactive in templates.
```sui
Counter is: {counter}
```
...
Settings are reactive in both reactions and templates.
...
const onCreated = ({settings, reaction}) => {
  ...
  settings.counter++; // reaction reruns
}
```
...
All [lifecycle events and callbacks](/docs/guides/components/lifecycle) will return `signal` and
...
> Learn more about controlling reactivity in the dedicated subsection for [controlling reactivity](/docs/guides/reactivity/controls)
