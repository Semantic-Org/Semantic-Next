...
Your component's `state` is the internal store of reactivity for your component. When your component is initialized each `state` property will become a [`signal`](/docs/guides/reactivity/signals) and will trigger reactivity inside a `Reaction`.
