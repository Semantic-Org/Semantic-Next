...
Key bindings can be attached to any component by passing in `keys` to `defineComponent`
...
In addition to the standard callback data found in [lifecycle guide](/docs/guides/components/lifecycle) keyboard bindings have access to a couple extra values.
...
All [lifecycle callbacks](/docs/guides/components/lifecycle) receive as a parameter two methods `bindKey` and `unbindKey` which can be used to dynamically add and remove keybindings as necessary.

This is often used with [component reactivity](/docs/guides/components/reactivity) to adjust key bindings when something significant has changed with a component.
...
<PlaygroundExample id="advanced-keybinding" height="500" direction="horizontal"></PlaygroundExample>
