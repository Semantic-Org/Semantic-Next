...
> *Layout Shift* - Be cautious using page css to define rules as they will not be present during server side rendering and can cause layout shifts until your component is defined.
...
When defining your component with `defineComponent` you can include a special callback that fires when the global theme changes for a page. This can be used to rerun javascript when the user selects a new theme.
...
Inside each [lifecycle callback](/docs/guides/components/lifecycle#callback-arguments), you can check the `darkMode` flag to determine if a dark mode theme is applied. This can be used to run different javascript for a dark mode theme.

#### Triggering Dark Mode

Dark mode will be set to `true` if `<html class="dark"></html>` is set to true for the element.
...
Dark mode can also be used selectively on light mode pages (and vice versa) using the css flag `--dark-mode`
```css
/* dark mode css tokens will apply and dark mode will trigger on component */
some-element {
  --dark-mode: true;
}
```

#### Changing Behaviors for Dark Mode

If dark mode is detected you can modify behaviors inside your component using the [lifecycle callback argument](/docs/guides/components/lifecycle#callback-arguments) `darkMode`.
...
To cause `onThemeChanged` to fire on your components you will need to dispatch a `themechange` event from the page.
...
This will allow you to apply arbitrary css not present in the original component.
