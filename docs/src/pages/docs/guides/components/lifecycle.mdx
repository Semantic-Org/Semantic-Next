...
Lifecycle callbacks (`onCreated`, `onRendered`, `onDestroyed`) fire on the server in the same order as on the client. [DOM lifecycle events](#dom-lifecycle-events) are suppressed during hydration so external listeners aren't fired twice for the same render. Use `isServer` or `isClient` inside callbacks when reaching for globals that may not be defined on the server.
