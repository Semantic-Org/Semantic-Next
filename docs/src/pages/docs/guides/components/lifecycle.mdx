...
* **[createComponent](#createcomponent)** - Defines your [component instance](/docs/guides/components/instances) that includes the invokable functionality of your component and exposed variables.
...
For a more detailed description of how callbacks are fired refer to the [callback event sequence diagram](/docs/guides/components/lifecycle#callback-sequence).

> **Render Function** Unlike many front end frameworks which require your render function to return the html for your component, your component's html is defined in the [template](/docs/api/templates) passed to `defineComponent`.
...
For more information see the dedicated section for [component instances](/docs/guides/components/instances).
...
Semantic UI components support server-side rendering out of the box. The default `native` rendering engine ships with a built-in SSR pipeline that emits Declarative Shadow DOM and hydrates without re-rendering on the client. Components using the optional `lit` engine can integrate with [Lit's SSR packages](https://lit.dev/docs/ssr/overview/#library-status) instead.
...
Lifecycle callbacks (`onCreated`, `onRendered`, `onDestroyed`) fire on the server in the same order as on the client.

[DOM lifecycle events](#dom-lifecycle-events) are suppressed during hydration — `created` and `rendered` events do not dispatch on the client when reusing server-rendered markup, so listeners outside the component aren't fired twice for the same render.

Use `isServer` or `isClient` inside callbacks when accessing globals that may not be defined on the server.
...
You can use these to do things like [query the DOM](/docs/guides/components/dom), setting up [custom reactivity](/docs/guides/components/reactivity#exposing-reactivity) and much more.

### Standard Arguments

The following parameters are destructurable from all callbacks.

> For information on `self` aliases see [self aliases](/docs/guides/components/instances#self-aliases).
...
[Key bindings](/docs/guides/components/keys) also have access to a few additional arguments
| parameter         | use                                              |
|-------------------|--------------------------------------------------|
...
>  For more information on custom events see the [dedicated subsection](/docs/guides/components/events)
