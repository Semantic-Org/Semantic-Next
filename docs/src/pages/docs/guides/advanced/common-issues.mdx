...
title: Common Web Component Issues
...
Use [`pageCSS`](/docs/guides/components/styling#pageCSS) to style from outside with tag names:
...
If a component is waiting for a bubbled event, like those that use [event delegation](/docs/guides/components/events) to attach events they will never 'see' the event being fired.

**Solution**

The [`dispatchEvent()`](/docs/guides/components/events#creating-events) utility automatically includes `{ bubbles: true, composed: true }`.
...
Templates provide a [special syntax for boolean expressions](/docs/guides/templates/expressions#boolean-attribute), without quotes. This means the attribute should be omitted entirely if the returned value is falsey.
...
[Templates](/docs/guides/templates/expressions) handle serialization automatically:
...
Use [`settings`](/docs/guides/query/components#settings) and [`initialize`](/docs/guides/query/components#initialize) from Query for imperative configuration after the component is in the page:
...
Use [subtemplates](/docs/guides/templates/subtemplates) for shared shadow root styling:
...
You may want to include [slots](/docs/guides/templates/slots) that may not always be filled in your web component. However, if these slots have styling they will always be present because `:blank` pseudoselector does not work with slots.
...
> **See**: [Settings vs Slots](/docs/guides/templates/slots#slots-vs-settings) for guidance on when to use each approach.
...
> **See**: [Settings vs Slots](/docs/guides/templates/slots#slots-vs-settings) for the complete decision framework and [wicg/webcomponents/936](https://github.com/WICG/webcomponents/issues/936) for related specification discussions.
