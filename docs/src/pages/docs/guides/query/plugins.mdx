...
The `$.plugin` object is an alias for `Query.prototype`. Methods added here become available on all Query instances with full access to [chaining](/docs/guides/query/chaining) and the Query API. Use plugins for utility methods that don't require state management or complex configuration.
...
Declarative event handlers with automatic [delegation](/docs/guides/query/basics#event-delegation):
...
- `'click .selector'` - [Delegated event](/docs/guides/query/basics#event-delegation) within behavior element
