...
Changes to [Signals](/docs/guides/reactivity/signals) do not cause dependent [Reactions](/docs/guides/reactivity/reactions) to run immediately. Instead, updates are queued and "flushed" together asynchronously. This batching process uses the browser's [microtask queue](https://developer.mozilla.org/en-US/docs/Web/API/HTML_DOM_API/Microtask_guide).
...
If you need your updates to propagate immediately you can use [`Reaction.flush()`](/docs/api/reactivity/reaction#flush).
...
To schedule a function to run *after* the next asynchronous flush completes, pass a callback to  [`Reaction.afterFlush()`](/docs/api/reactivity/reaction#afterflush).
