...
If you need your updates to propagate immediately you can use [`flush()`](/docs/api/reactivity/flushing#flush).
...
To schedule a function to run *after* the next asynchronous flush completes, pass a callback to  [`afterFlush()`](/docs/api/reactivity/flushing#afterflush).
